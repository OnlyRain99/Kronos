...
/*function matchsched($theemp, $dblogin, $dbbreo, $dbbrei, $dblogo){
    ...
    if(mysqli_num_rows($empsch) > 0){
        while ($scrow=$empsch->fetch_array()) {
            ...
            if(strtotime($dblogin) < $schedlout && strtotime($endday) >= $schedin || $sqlemp=="`gy_sched_day`='".$datenow."'"){
    ...
    if($error == 0){
        ...
        if($type != 8 && ($type !=6 || ($status != 2&&$status != 3))){
            ...
            if($type==6&&$status==1){
                ...
                if(date("Y-m-d H:i", strtotime($logout))>date("Y-m-d H:i", strtotime($gytlo))&& (($gysli<=$gyslo&&date("Y-m-d H:i", strtotime($logout))>date("Y-m-d H:i", strtotime($escdate." ".$gyslo)))||($gysli>$gyslo&&date("Y-m-d H:i", strtotime($logout))>date("Y-m-d H:i", strtotime($escdate." ".$gyslo."+1 day")))) ){ $error++; $daerr.=" Post Value Exceeded "; }
            ...
            if($status == 0){
                ...
                if($status==2 && $breakout!="" && $breakin!=""){
                  ...
                  if($type==6){
					...
					if(date("Y-m-d H:i", strtotime($newout))>date("Y-m-d H:i", strtotime($oldout))&& (($gysli<=$gyslo&&date("Y-m-d H:i", strtotime($newout))>date("Y-m-d H:i", strtotime($escdate." ".$gyslo)))||($gysli>$gyslo&&date("Y-m-d H:i", strtotime($newout))>date("Y-m-d H:i", strtotime($escdate." ".$gyslo."+1 day")))) ){ $error++; $daerr.=" Post Value Exceeded"; }
