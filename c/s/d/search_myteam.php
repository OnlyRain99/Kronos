...
function gettmpsup($empcd, $crdate){
	include '../../../config/conn.php';
	$tsid="";
	$tssql=$link->query("SELECT `gy_temp_sup`.`temp_sup_id`as`ts_id` FROM `gy_temp_sup` LEFT JOIN `gy_employee` ON `gy_temp_sup`.`temp_sup_code`=`gy_employee`.`gy_emp_code` WHERE `gy_temp_sup`.`temp_sup_code`='$empcd' AND `gy_temp_sup`.`temp_sup_date`='$crdate' AND `gy_temp_sup`.`temp_sup_by`=`gy_employee`.`gy_emp_supervisor` ");
...
$tmpsup = gettmpsup($user_code, $onlydate);
...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='".$nameid."' ".$sqlseq." Order By `gy_tracker_date` ASC");
	while ($tsrow=$tmsht->fetch_array()) {
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			tblcntt($thismode, $tsrow['gy_emp_fullname'], date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $cnttruedate[2], $cnttruedate[3], $cnttruedate[1], 0, 0, $undertime, 0, wordlimit($tsrow['gy_emp_account'], 12), "No Logs", 0, "", "", "", "", "", $tsrow['gy_emp_code'], $tmpsup);
 ...
 function tblcntt($mode, $name, $date, $in, $out, $bout, $bin, $si, $so, $wh, $bh, $ot, $utl, $ath, $account, $omname, $om, $request, $trackid, $reason, $remarks, $history, $empcode, $tmpsup){
 $myom = "";
