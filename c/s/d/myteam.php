...
$tsid="";
$tssql=$link->query("SELECT `gy_temp_sup`.`temp_sup_id`as`ts_id` FROM `gy_temp_sup` LEFT JOIN `gy_employee` ON `gy_temp_sup`.`temp_sup_code`=`gy_employee`.`gy_emp_code` WHERE `gy_temp_sup`.`temp_sup_code`='$user_code' AND `gy_temp_sup`.`temp_sup_date`='$onlydate' AND `gy_temp_sup`.`temp_sup_by`=`gy_employee`.`gy_emp_supervisor` ");
                        ...
                        <div class="row">
                            <div class="col-lg-3">
                                ...
                                <select name="selectnames" id="selectnames" class="form-control" onchange="procsearch('Loading...')">
                                	<option></option>
                                	<?php
                                	$agtnm=$link->query("SELECT `gy_emp_fullname`,`gy_emp_code` From `gy_employee` LEFT JOIN `gy_user` ON `gy_employee`.`gy_emp_code`=`gy_user`.`gy_user_code` Where `gy_user`.`gy_user_status`=0 AND `gy_emp_supervisor`='$user_id'");
    ...
    <script type="text/javascript">
        ...
        xhttp.onreadystatechange = function(){
            if (this.readyState == 4 && this.status == 200){
				...
				else if(status=="duporerr"){ status="warning"; title="DTR logs is invalid, please check if the logs is complete or the date is not duplicated to the already approved date."; }
        ...
        function approvedalllogs(empcode, arrckd, i1, ahtnoot){
            ...
            xhttp.open("POST", "approveallchk.php", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
