...
<body>
    ...
    if(date("d")<=3){ $datestrt = date("Y-m-16", strtotime("-1 Month")); }
        ...
        $today = date("Y-m-d");
            ...
            $i = 0;
                if($user_type>=15){
                    $schesql = $link->query("SELECT * From `gy_schedule_escalate` LEFT JOIN `gy_user` ON `gy_schedule_escalate`.`gy_emp_code`=`gy_user`.`gy_user_code` Where (`gy_req_by`='$user_id' OR `gy_req_to`='$user_id' OR `gy_sup`='$user_id' OR `gy_emp_code`='$user_code') AND `gy_req_status`='0' AND `gy_user`.`gy_user_type`>=6 AND `gy_schedule_escalate`.`gy_sched_day`>='$datestrt' ORDER BY `gy_sched_esc_id` desc");
                }else{
                    $schesql = $link->query("SELECT * From `gy_schedule_escalate` Where (`gy_req_by`='$user_id' OR `gy_req_to`='$user_id' OR `gy_sup`='$user_id' OR `gy_emp_code`='$user_code') AND `gy_req_status`='0' AND `gy_schedule_escalate`.`gy_sched_day`>='$datestrt' ORDER BY `gy_sched_esc_id` desc");
                }
                ...
                if($user_type>=15){
                    $logssql = $link->query("SELECT * From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` LEFT JOIN `gy_user` ON `gy_escalate`.`gy_usercode`=`gy_user`.`gy_user_code` Where (`gy_esc_by`='$user_id' OR `gy_esc_to`='$user_id' OR `gy_sup`='$user_id' OR `gy_tracker`.`gy_emp_code`='$user_code') AND `gy_esc_status`='0' AND `gy_user`.`gy_user_type`>=6 AND `gy_escalate`.`gy_tracker_date`>='$datestrt' ORDER BY `gy_esc_id` desc");
                }else{
                ...
                //loa Pending
                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` LEFT JOIN `gy_accounts` ON `gy_leave`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` LEFT JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_acc_id`=$myaccount OR `gy_employee`.`gy_emp_supervisor`=$user_id OR `gy_accounts`.`gy_dept_id`=$user_dept) AND (`gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']." OR `gy_leave`.`gy_user_id`=$user_id) AND `gy_leave`.`gy_leave_status`=0 ");
            ...
            <?php }} function cardcontent($cardn, $esct, $own, $reqdate, $empid, $fname, $reqstatus, $reqdeny, $reqby, $reqto, $sup, $schmode, $schday, $schli, $schbo, $schbi, $schlo, $reqreson, $msguc, $rfid, $publish, $ucode){
                ...
                <div class="card hover-overlay shadow rounded">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <div class="position-relative m-4">
                            ...
                            <?php if($own==0){ ?>
                            <button type="button" class="position-absolute top-0 start-0 translate-middle btn btn-sm <?php echo "btn-".$color; ?> rounded-pill" style="width: 2rem; height:2rem;" data-bs-toggle="popover" tabindex="0" data-trigger="focus" data-bs-placement="left" title="Employee Requested" data-bs-content="For <?php echo str_replace('"', '\\"', $fname); ?>">
                                <i class='fas fa-street-view'></i>
                            </button>                                
                            <?php } ?>
                            <button type="button" class="position-absolute top-0 start-<?php if(($own==0 && $reqstatus==0)||($reqstatus!=0&&$msguc!=""&&$publish==0)){echo "33";}else if($own==0 && $reqstatus!=0){echo "50";}else{echo "0"; } ?> translate-middle btn btn-sm <?php echo "btn-".$color; ?> rounded-pill" style="width: 2rem; height:2rem;" data-bs-toggle="popover" data-bs-placement="top" title="<?php echo $escname; ?>" data-bs-content="Requested By <?php echo str_replace('"', '\\"', getuserfullname($reqby)); ?>">
                                <i class='far fa-user'></i>
                            </button>
                            <?php if($reqstatus==0){ ?>
                            ...
                            <?php } ?>
                            <button type="button" class="position-absolute top-0 start-<?php if($reqstatus!=0&&$msguc!=""&&$publish==0){echo "66";}else{echo "100"; } ?> translate-middle btn btn-sm <?php if($color=="warning"){echo "btn-secondary";}else{ echo "btn-".$color; } ?> rounded-pill" style="width: 2rem; height:2rem;" <?php if($reqstatus!=0){ ?> data-bs-toggle="popover" data-bs-placement="right" title="<?php echo $fstatus; ?>" data-bs-content="<?php echo str_replace('"', '\\"', $fmess); ?>" <?php } ?>>
                                <?php echo $finalicon; ?>
                            </button>
                    ...
                    <div class="card-footer <?php echo $mask; ?>">
                        ...
                        <div class="pull-right">
                            <span class="btn-outline-dark" style="cursor: pointer;" data-bs-toggle="popover" data-bs-placement="left" title="Timekeep Requested" data-bs-content="<?php echo str_replace('"', '\\"', $nlog); ?>"><i class='fas fa-clock'></i></span>
                            <span class="btn-outline-dark" style="cursor: pointer;" data-bs-toggle="popover" data-bs-placement="bottom" title="Breaktime Requested" data-bs-content="<?php echo str_replace('"', '\\"', $nbreak); ?>"><i class='far fa-clock'></i></span>
                            <span class="btn-outline-dark" style="cursor: pointer;" data-bs-toggle="popover" data-bs-placement="right" title="The Reason" data-bs-content="<?php echo str_replace('"', '\\"', $reqreson); ?>"><i class='fas fa-exclamation-circle'></i></span>
