...
$ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` LEFT JOIN `gy_accounts` ON `gy_leave`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` LEFT JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE `gy_leave`.`gy_user_id`!='$user_id' AND `gy_employee`.`gy_emp_supervisor`='$user_id' OR ((`gy_accounts`.`gy_dept_id`='$user_dept' AND `gy_accounts`.`gy_dept_id`!=1)OR(`gy_accounts`.`gy_dept_id`=1 AND `gy_leave`.`gy_acc_id` IN (".implode(',',$acntarr)."))) AND `gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']);
   ...
   //$acntarr[$i9]=$myaccount; $i9++;
    ...
    $levsql=$link->query("SELECT * From `gy_leave_available` LEFT JOIN `gy_accounts` on `gy_leave_available`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` WHERE `gy_leave_available`.`gy_acc_id` IN (".implode(',',$acntarr).") OR (`gy_accounts`.`gy_dept_id`=$user_dept AND `gy_accounts`.`gy_dept_id`!=1) order by `gy_leave_available`.`gy_leave_avail_date` asc");
	...
	for($i3=0;$i3<$i1;$i3++){if($lvearr[$i][0]==$dptarr[$i3][0]){$title=$dptarr[$i3][2]; break;}}
    	...
    	echo '"description": "('.$lvearr[$i][7].') '.'('.$lvearr[$i][2].'/'.$lvearr[$i][3].') '.$lvearr[$i][6].'",';
		echo '"color": "#D4AC0D"';
