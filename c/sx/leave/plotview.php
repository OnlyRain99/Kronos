...
//$ifreqalr = ifreqalready($startdate, $user_id);
...
$wtdsql=$link->query("SELECT * From `gy_leave_available` LEFT JOIN `gy_accounts` on `gy_leave_available`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` Where (`gy_leave_available`.`gy_acc_id` IN (".implode(',',$acntarr).") OR (`gy_accounts`.`gy_dept_id`=$user_dept AND `gy_accounts`.`gy_dept_id`!=1)) and `gy_leave_available`.`gy_leave_avail_date`<='$thisdate' AND `gy_leave_available`.`gy_leave_avail_dateto`>='$thisdate' ORDER BY `gy_leave_available`.`gy_leave_avail_date` asc");
...
<?php }else if($ifreqalr>=1 && ($pltvw==3 || $pltvw!=4) ){ ?>
            ...
            <div class="card-body">
                ...
                <table class="table table-responsive table-striped">
                    <tbody>
                        ...
                        <?php } ?>
                        <!--<tr>
                            <th scope="row" class="text-nowrap text-right">Remarks :</th>
                            <td class="text-left"><?php// echo $dssarr[$i4][4]; ?></td>
                        </tr>-->
                    </tbody>
                </table>
                ...
                <table class="table table-responsive table-striped">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-left"><?php $lslot=0;  for($i5=0;$i5<$i;$i5++){ if($wtdarr[$i5][7]==$ctlrow['gy_acc_id'] && $wtdarr[$i5][1]<=$ctlrow['gy_leave_date_from'] && $wtdarr[$i5][2]>=$ctlrow['gy_leave_date_from']){ $lslot=$wtdarr[$i5][3]-$wtdarr[$i5][4]; break; } } if($lslot<=0){echo'No';}else{echo $lslot;} echo' Remaining'; ?></td>
                        </tr>
