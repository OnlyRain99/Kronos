...
if(($levcnt>0 && $levrow['gy_leave_avail_approved']<$levrow['gy_leave_avail_plotted'])||($leave['gy_leave_type']==2||$leave['gy_leave_type']==9)){
$levavailid=$levrow['gy_leave_avail_id'];

    $update_leave=$link->query("UPDATE `gy_leave` SET `gy_leave_status`='1', `gy_leave_approver`='$user_id',`gy_leave_date_approved`='$datenow',`gy_leave_remarks`='approved' Where `gy_user_id`!='$user_id' AND `gy_acc_id`='".$leave['gy_acc_id']."' AND `gy_leave_date_from`='$thisdate' AND `gy_leave_id`='$lid'");
    if($update_leave){
