...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='".$nameid."' ".$sqlseq." Order By `gy_tracker_date` ASC");
	while ($tsrow=$tmsht->fetch_array()) {
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			tblcntt($thismode, $tsrow['gy_emp_fullname'], date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $cnttruedate[2], $cnttruedate[3], $cnttruedate[1], 0, 0, $undertime, 0, "No Logs", 0, "", "", "", "", "", $tsrow['gy_emp_code']);
 ...
 function tblcntt($mode, $name, $date, $in, $out, $bout, $bin, $si, $so, $wh, $bh, $ot, $utl, $ath, $omname, $om, $request, $trackid, $reason, $remarks, $history, $empcode){
 $myom = "";
...
<td style="padding: 1px;" class="text-center">
    ...
    //temp->
    //if($empcode==4847 && date("Y-m-d")=="2023-04-07"){ $today=date("m/d/Y", strtotime($today.' -1 day')); }
    //<-temp
