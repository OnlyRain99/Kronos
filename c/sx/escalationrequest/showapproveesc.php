...
$snum = ($pgnm * $numofpg) - $numofpg;
	...
	$sql1="SELECT * From `gy_schedule_escalate` LEFT JOIN `gy_employee` ON `gy_schedule_escalate`.`gy_emp_code`=`gy_employee`.`gy_emp_code` Where `gy_schedule_escalate`.`gy_req_status`='1' AND `gy_schedule_escalate`.`gy_publish`=0 AND `gy_employee`.`gy_emp_supervisor`=$user_id Order By `gy_schedule_escalate`.`gy_req_date` DESC, `gy_schedule_escalate`.`gy_sched_mode` ASC";
    $sql2="SELECT * From `gy_escalate` LEFT JOIN `gy_employee` ON `gy_escalate`.`gy_usercode`=`gy_employee`.`gy_emp_code` Where `gy_escalate`.`gy_esc_status`='1' AND `gy_escalate`.`gy_publish`=0 AND `gy_employee`.`gy_emp_supervisor`=$user_id Order By `gy_esc_date` DESC, `gy_escalate`.`gy_esc_type` ASC";
