...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='$user_code' ".$sqlseq." Order By `gy_tracker_date` ASC");
	while ($tsrow=$tmsht->fetch_array()) {
		...
		if($user_type==10 || $user_type>14){ $truedate[1]=getwh($tsrow['gy_tracker_login'], $tsrow['gy_tracker_logout']); }
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			tblcntt($thismode, date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $cnttruedate[1], 0, 0, $undertime, 0, "No Logs", 0, "", "", "", "", "", $tsrow['gy_emp_code'], $user_type);
	...
	while($cntdate<=date("Y-m-d", strtotime($dateto))){
		...
		tblcntt($thismode, date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $truedate[1], 0, 0, $undertime, 0, "No Logs", 0, "", "", "", "", "", $user_code, $user_type);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
	if($i == 0){ echo "No Logs Found"; }
    $link->close();
 function tblcntt($mode, $date, $in, $out, $bout, $bin, $wh, $bh, $ot, $utl, $ath, $omname, $om, $request, $trackid, $reason, $remarks, $history, $sbsid, $lv){
 $myom = "";
