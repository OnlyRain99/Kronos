...
while($tmsrow=$tmsht->fetch_array()){
	...
	while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
		...
		tblcntt($cnttruedate[4], $tmsrow['gy_emp_code'], $userarr[0][$i1], $userarr[2][$i1], $cntdate, $cnttruedate[2], $cnttruedate[3], "", "", "", "", 0);
...
if(date("Y-m-d", strtotime($truedate[0]))>=date('Y-m-d', strtotime($fdate)) && date("Y-m-d", strtotime($truedate[0]))<=date('Y-m-d', strtotime($tdate))){
	...
	tblcntt($truedate[4], $tmsrow['gy_emp_code'], $userarr[0][$i1], $userarr[2][$i1], $truedate[0], $truedate[2], $truedate[3], $tmsrow['gy_tracker_login'], $tmsrow['gy_tracker_logout'], $tmsrow['gy_tracker_breakout'], $tmsrow['gy_tracker_breakin'], $tmsrow['gy_tracker_bh']);
}
	$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
}
	while($cntdate<=date("Y-m-d", strtotime($tdate))){
		...
		tblcntt($truedate[4], $usrarr[$i1], $userarr[0][$i1], $userarr[2][$i1], $cntdate, $truedate[2], $truedate[3], "", "", "", "", 0);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
}
} $link->close();
...
<?php function tblcntt($mod, $id, $name, $accnm, $date, $schin, $schout, $login, $logout, $bout, $bin, $bh){
	$ah=0; $th=0;
