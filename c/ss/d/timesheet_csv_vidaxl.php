...
$fields = array('Agent Name', 'Date', 'Total transaction', 'Team Lead', 'Schedule In', 'Schedule Out', 'Actual Log In', 'Actual Log Out','Day','Total Hour','Total Hour(LB)');
...
function tblcntt($mod, $name, $date, $th, $tl, $tr, $schin, $schout, $login, $logout, $bh){
...
$thlb = gettwh($th-$bh);
$lineData = array($name, date("m/d/Y", strtotime($date)), $tr, $tl, $schin, $schout, $login, $logout, date("D", strtotime($date)), $th, $thlb);
...
for($i1=0;$i1<count($usrarr);$i1++){
	...
	while($tmsrow=$tmsht->fetch_array()){
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			$lineData = tblcntt($cnttruedate[4], $userarr[0][$i1], $cntdate, $cnttruedate[1], $userarr[1][$i1], 0, chktime($cnttruedate[2]), chktime($cnttruedate[3]), "", "", 0);
			fputcsv($fp, $lineData);
		...
		$lineData = tblcntt($truedate[4], $userarr[0][$i1], $truedate[0], $truedate[1], $userarr[1][$i1], $tracnt, chktime($truedate[2]), chktime($truedate[3]), chktime($tmsrow['gy_tracker_login']), chktime($tmsrow['gy_tracker_logout']), $tmsrow['gy_tracker_bh']);
		fputcsv($fp, $lineData);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
	while($cntdate<=date("Y-m-d", strtotime($tdate))){
		...
		$lineData = tblcntt($truedate[4], $userarr[0][$i1], $cntdate, $truedate[1], $userarr[1][$i1], 0, chktime($truedate[2]), chktime($truedate[3]), "", "", 0);
		fputcsv($fp, $lineData);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
}
