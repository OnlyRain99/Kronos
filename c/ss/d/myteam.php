                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3">
                                ...
                                <select name="selectnames0" id="selectnames0" class="form-control" onchange="updtmysup(this)" required>
                                    <option></option>
                                    <?php
                                    $agtnm=$link->query("SELECT `gy_emp_fullname`,`gy_emp_code`,`gy_emp_account` From `gy_employee` LEFT JOIN `gy_user` ON `gy_employee`.`gy_emp_code`=`gy_user`.`gy_user_code` Where `gy_user`.`gy_user_status`=0 AND (`gy_employee`.`gy_emp_supervisor`='$user_id' OR (`gy_employee`.`gy_emp_type`=2 AND `gy_employee`.`gy_acc_id`='$myaccount'))");
                                    while ($agtlst=$agtnm->fetch_array()){ ?>
                                    <option value="<?= $agtlst['gy_emp_code']; ?>" title="<?php echo $agtlst['gy_emp_account']; ?>" <?php if($supcode==$agtlst['gy_emp_code'] || $daagent==$agtlst['gy_emp_code']){ echo "selected"; }?>><?= $agtlst['gy_emp_fullname']; ?></option>
                                    <?php } ?>
                                </select>
                                </div>
                            </div>
                            	...
                            	<form method="post" enctype="multipart/form-data">
                            		...
                            		<thead>
                                        <tr class="mybg" id="thisisthetableheadid">
                                            ...
                                            <?php if(false){ ?>
                                            <th style="padding: 10px;" class="text-center faa-parent animated-hover" name="hidifnotl2"><i class="fa-solid fa-gavel faa-wrench"></i></th><?php } ?>
                                        </tr>
                                    </thead>
    ...
    <script type="text/javascript">
		...
		function switchdsply(){
			...
			if(theswitchval==0){
                ...
                '<th style="padding: 10px;" class="text-center"><i class="fa fa-thumbs-down"></i></th>'+
                '<th style="padding: 10px;" class="text-center"><i class="fa fa-arrow-circle-up"></i></th>'+
                '<th style="padding: 10px;" class="text-center faa-parent animated-hover" name="hidifnotl2"><i class="fa-solid fa-gavel faa-wrench"></i></th>';
        ...
        function proctsact(elem, tsid, tsdate, tscode){
            var loc = "";
            var pstsnd = "";
            if(tsid==""){
                loc = "team/new_tmpsup.php";
                pstsnd = "tsdate="+tsdate+"&tscode="+tscode;
            }else{
                loc = "team/remove_tmpsup.php";
                pstsnd = "tsid="+tsid+"&tscode="+tscode;
            }
            var xhttp = new XMLHttpRequest();
                xhttp.onreadystatechange = function(){
                if (this.readyState == 4 && this.status == 200){
                    var tstxt = this.responseText;
                    if(tstxt=="tsnew" || tstxt=="tsremove"){ elem.innerHTML='<i class="fa fa-refresh fa-spin"></i>'; procsearch(''); }
                }};
        ...
        xhttp.open("GET", "sb_issueres.php", true);
        xhttp.send();
		}
        ...
        xhttp.open("POST", "search_members.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		...
		var loc = "search_myteam.php";
		var switchval = document.getElementById("hiddsplytyp").value;
		if(switchval==0){ loc="search_myteam.php"; }else if(switchval==1){ loc="team/search_thesched.php"; }
		var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function(){
            if (this.readyState == 4 && this.status == 200){
                ...
                var e = document.getElementById("selectnames");
                var tblcl = document.getElementsByName("hidifnotl2");
                for(var cls of tblcl){
                    if(e.value!="" && e.selectedIndex==0){
                        cls.classList.remove("visually-hidden");
                    }else{
                        cls.classList.add("visually-hidden");
                    }
                }
            }
        };
        ...
        xhttp.open("POST", "approvedtr.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        ...
        xhttp.open("POST", "rejectdtr.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
