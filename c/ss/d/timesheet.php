...
if($user_type == 3 || $user_type == 4 || $user_type == 18 || $user_type == 10){
	...
	$lstmnthf=date("Y-m-d", strtotime("first day of last month"));
	$lstmntht=date("Y-m-d", strtotime("last day of last month"));
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="input-group-prepend">
    <div class="form-floating minwid-120">
        <select class="form-select" id="slt_depact" onchange="upd_usrlst(this)" required>
            ...
            <?php $depsql=$link->query("SELECT `gy_acc_id`,`gy_acc_name` From `gy_accounts` Where `gy_acc_id` IN (".implode(',',$acntarr).") ORDER BY `gy_acc_name` ASC");
    ...
    <div class="form-floating">
        <input type="date" name="from" id="datefrom" onchange="daterange()" value="<?php echo $lstmnthf; ?>" class="form-control" required>
        <label class="datefrom">Date From</label>
    </div>
    <div class="form-floating">
        <input type="date" name="to" id="dateto" onchange="daterange()" value="<?php echo $lstmntht; ?>" class="form-control" required>
