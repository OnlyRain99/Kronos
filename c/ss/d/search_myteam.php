...
function gettmpsup($empcd, $crdate, $supid){
	include '../../../config/conn.php';
	$tsid = "";
	$tssql=$link->query("SELECT `gy_temp_sup`.`temp_sup_id`as`ts_id` FROM `gy_temp_sup` LEFT JOIN `gy_employee` ON `gy_temp_sup`.`temp_sup_code`=`gy_employee`.`gy_emp_code` WHERE `gy_temp_sup`.`temp_sup_code`='$empcd' AND `gy_temp_sup`.`temp_sup_date`='$crdate' AND `gy_temp_sup`.`temp_sup_by`='$supid' ");
	while ($tsrow=$tssql->fetch_array()){
		$tsid = $tsrow['ts_id'];
	}
	$link->close();
	return $tsid;
...
function ifomname($omname){
	if($omname != "Pending" && $omname != "No Logs" && $omname != "RDOT"){ return "disabled"; }
}
...
$ifcor=$link->query("SELECT `gy_emp_code` From `gy_employee` Where (`gy_acc_id`='$myaccount' OR `gy_emp_supervisor`='$user_id') AND `gy_emp_code`='$nameid' LIMIT 1");
...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='".$nameid."' ".$sqlseq." Order By `gy_tracker_date` ASC");
	while ($tsrow=$tmsht->fetch_array()) {
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
				...
				$tmpsup=gettmpsup($tsrow['gy_emp_code'], date("Y-m-d", strtotime($cntdate)), $user_id);
			tblcntt($thismode, $tsrow['gy_emp_fullname'], date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $cnttruedate[2], $cnttruedate[3], $cnttruedate[1], 0, 0, $undertime, 0, "No Logs", 0, "", "", "", "", "", $tsrow['gy_emp_code'], $tmpsup);
	...
	$tmpdate = $cntdate;
		 ...
		 while($cntdate<=date("Y-m-d", strtotime($dateto))){
            ...
            $truedate = matchsched($nameid, $cntdate." 00:00:00", "0000-00-00 00:00:00", "0000-00-00 00:00:00", "0000-00-00 00:00:00");
		 	...
		 	$tmpsup=gettmpsup($nameid, date("Y-m-d", strtotime($cntdate)), $user_id);
			tblcntt($thismode, $fullname, date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", "No Log", "No Log", $truedate[2], $truedate[3], $truedate[1], 0, 0, $undertime, 0, "No Logs", 0, "", "", "", "", "", $nameid, $tmpsup);
		 $cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
		 }

	if($i == 0){ echo "No Logs Found"; }
}else{ echo "Invalid Access!"; }    $link->close();
 function tblcntt($mode, $name, $date, $in, $out, $bout, $bin, $si, $so, $wh, $bh, $ot, $utl, $ath, $omname, $om, $request, $trackid, $reason, $remarks, $history, $empcode, $tmpsup){
 $myom = "";
 ...
 if($mode==0){
     ...
     if($si==''){$si='OFF';} 
     if($so==''){$so='OFF';}
     $wh=0; $bh=0; $ot=0; $omname='RD'; $txtcolor=array_fill(1, 5, 'text-blue'); $txtcolor[0] = ''; $wh = 0;
     if($request == "escalate"){ $omname = "Escalating <i class='fa fa-arrow-up'></i>"; }}
...
?>
<tr class="mybg">
<td style="padding: 5px;" class="text-center text-nowrap <?php echo $txtcolor[1]; ?>"><?php echo $name; ?></td>
...
<td style="padding: 1px;" class="text-center">
	<?php if($trackid!=""&&chktime($out)!="--:--"&&chktime($si)!="--:--"&&chktime($so)!="--:--"){?>
	<button id="c3_<?php echo $trackid; ?>" type="button" data-toggle="modal" data-target="#app_<?php echo $trackid; ?>" class="btn btn-success btn-sm btn-block" title="click to approve ..." <?php echo ifomname($omname); ?> ><i class="fa fa-thumbs-up"></i></button>
	<?php } ?>
</td>
<td style="padding: 1px;" class="text-center">
	<?php if($trackid!=""&&chktime($out)!="--:--"&&chktime($si)!="--:--"&&chktime($so)!="--:--"){?>
	<button id="c4_<?php echo $trackid; ?>" type="button" data-toggle="modal" data-target="#rej_<?php echo $trackid; ?>" class="btn btn-danger btn-sm btn-block" title="click to reject ..." <?php echo ifomname($omname); ?> ><i class="fa fa-thumbs-down"></i></button>
	<?php } ?>
</td>
<td style="padding: 1px;" class="text-center">
	<?php
		...
		$today = date("m/d/Y");
			while($ntype < 3){
				$lday = date("m/d/Y", strtotime($today.' -1 day'));
				//if(date("w", strtotime($lday)) != 0 && date("w", strtotime($lday)) != 6){ $ntype++; }
				$ntype++;
				$today = $lday;
			}
	if(strtotime($date) >= strtotime($today) && (strtotime($date) < strtotime(date("m/d/Y")) || (strtotime($date)==strtotime(date("m/d/Y"))&&chktime($out)!="--:--"))){
	if($wh<=0 && ($omname == "Pending" || $omname == "No Logs" || $omname == "RD")&&(strtotime($date)<strtotime(date("m/d/Y")))){ if($trackid==""){ $trackid=0; } ?>
    <button type="button" <?php echo "onclick='escatrig(this, ".$trackid.", \"".$date."\")'"; ?> class="btn btn-info btn-sm btn-block" title="Escalate Schedule Adjustment" name="btnesa"><i class="fa fa-arrow-circle-up"></i></button>
    <?php }else if($trackid!="" || $wh > 0){ if($trackid==""){ $trackid=0; } if($request=="escalate"){ ?>
    <button type="button" class="btn btn-danger btn-sm btn-block" title="Cancel Escalation Request" onclick="cancelesc(<?php echo $trackid; ?>, '<?php echo $date; ?>')"><i class="fa-solid fa-rectangle-xmark"></i></button>
    <?php }else{ ?>
	<button type="button" <?php if(ifomname($omname) != "disabled"){ echo "onclick='escatrig(this, ".$trackid.", \"".$date."\")'"; } ?> class="btn btn-info btn-sm btn-block" title="click to escalate ..." <?php if($omname=="No Logs"&&strtotime($date)==strtotime(date("m/d/Y"))){?>name="btnesa"<?php }else{ ?>name="btneml" <?php } echo ifomname($omname); ?>><i class="fa fa-arrow-circle-up"></i></button>
	<?php }}}else if($request=="escalate"){ ?><button type="button" class="btn btn-danger btn-sm btn-block" title="Cancel Escalation Request" onclick="cancelesc(<?php echo $trackid; ?>, '<?php echo $date; ?>')"><i class="fa-solid fa-rectangle-xmark"></i></button><?php } ?>
</td>
<?php if(false){ ?>
<td style="padding: 1px;" class="text-center" name="hidifnotl2">
	<?php if(date("Y-m-d", strtotime($date))>=date("Y-m-d")){ if($tmpsup!=""){ ?>
	<button type="button" class="btn btn-success btn-sm btn-block faa-parent animated-hover" title="This employee has given permission to escalate a request on this day" id="tsactive" onclick="proctsact(this, '<?php echo $tmpsup; ?>', '<?php echo date("Y-m-d", strtotime($date)); ?>', '<?php echo $empcode; ?>')" ><i class="fa-solid fa-person-circle-check faa-pulse"></i></button>
	<?php }else{ ?>
	<button type="button" class="btn btn-secondary btn-sm btn-block faa-parent animated-hover" title="Enable escalation access on this day?" id="tsnotactive" onclick="proctsact(this, '<?php echo $tmpsup; ?>', '<?php echo date("Y-m-d", strtotime($date)); ?>', '<?php echo $empcode; ?>')"><i class="fa-solid fa-user-tie faa-float"></i></button>
	<?php }}else if($tmpsup!=""){ ?>
	<button type="button" class="btn btn-success btn-sm btn-block faa-parent animated-hover" title="This employee has given permission to escalate a request on this day" disabled><i class="fa-solid fa-person-circle-check faa-pulse"></i></button>
	<?php } ?>
</td>
<?php } ?>
</tr>
                        ...
                        <div class="form-group">
                            ...
                            <label style="font-size: 12px;">
                                <?php 
                                    ...
                                    foreach ($history as $logs) {  echo $logs."<br>"; }
                                ?>
                            </label>
                        </div>
                    </div>
...
<?php } if($trackid!="" && ifomname($omname)!= "disabled" && strtotime($date) <= strtotime(date("m/d/Y"))){ ?>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" name="wh" id="wh_<?php echo $trackid; ?>" value="<?php if($theath>8){echo '8';}else{echo $theath;} ?>" <?php if($theath <= gettwh($wh - $bh)){ echo "readonly"; } ?> min="0" max="<?php echo $theath; ?>" oninput="<?php echo "check_input(".$trackid.")"; ?>" required>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" onclick="<?php echo "approvetrig('".$trackid."')"; ?>" data-dismiss="modal" name="approve" id="submit" class="btn btn-success">Confirm</button>
            </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="rej_<?php echo $trackid; ?>" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                <btn data-dismiss="modal" onclick="<?php echo "rejecttrig('".$trackid."')"; ?>" name="update" class="btn btn-danger">Confirm</btn>
            </div>
            </form>
        </div>
    </div>
