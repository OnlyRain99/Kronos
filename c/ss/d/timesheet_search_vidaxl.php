...
while($tmsrow=$tmsht->fetch_array()){
	...
	while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
		...
		tblcntt($cnttruedate[4], $userarr[0][$i1], $cntdate, $cnttruedate[1], $userarr[1][$i1], 0, $cnttruedate[2], $cnttruedate[3], "", "", 0);
...
if(date("Y-m-d", strtotime($truedate[0]))>=date('Y-m-d', strtotime($fdate)) && date("Y-m-d", strtotime($truedate[0]))<=date('Y-m-d', strtotime($tdate))){
	...
	tblcntt($truedate[4], $userarr[0][$i1], $truedate[0], $truedate[1], $userarr[1][$i1], $tracnt, $truedate[2], $truedate[3], $tmsrow['gy_tracker_login'], $tmsrow['gy_tracker_logout'], $tmsrow['gy_tracker_bh']);
}
	$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
}
	while($cntdate<=date("Y-m-d", strtotime($tdate))){
		...
		tblcntt($truedate[4], $userarr[0][$i1], $cntdate, $truedate[1], $userarr[1][$i1], 0, $truedate[2], $truedate[3], "", "", 0);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
}
$link->close();
...
<?php function tblcntt($mod, $name, $date, $th, $tl, $tr, $schin, $schout, $login, $logout, $bh){
	$ccolor = ""; if($mod==0){ $schin="OFF"; $schout="OFF"; $login="OFF"; $logout="OFF"; $ccolor="bg-warning"; }
	if(($schin!="" &&  $schout!="") && ($login == "0000-00-00 00:00:00" || $login == "" || $logout == "0000-00-00 00:00:00" || $logout == "") && strtotime($date) < strtotime(date("Y-m-d"))){ $ccolor="bg-warning"; $login="ABSENT"; $logout="ABSENT"; $th=0; $bh=0; }
