<?php  
    ...
    $title = "Leave Request History Search: ".$search_text;

    $my_emps = getall_leave_level3($user_id);

    $query_one = "SELECT * From `gy_leave` LEFT JOIN `gy_user` On `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` Where (`gy_user`.`gy_full_name` LIKE '%$search_text%') AND `gy_leave_status`!='0' AND `gy_leave`.`gy_user_id` IN ('".$my_emps."') Order By `gy_leave_filed` DESC";

    $query_two = "SELECT COUNT(`gy_leave_id`) From `gy_leave` LEFT JOIN `gy_user` On `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` Where (`gy_user`.`gy_full_name` LIKE '%$search_text%') AND `gy_leave_status`!='0' AND `gy_leave`.`gy_user_id` IN ('".$my_emps."') Order By `gy_leave_filed` DESC";

    $query_three = "SELECT * From `gy_leave` LEFT JOIN `gy_user` On `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` Where (`gy_user`.`gy_full_name` LIKE '%$search_text%') AND `gy_leave_status`!='0' AND `gy_leave`.`gy_user_id` IN ('".$my_emps."') Order By `gy_leave_filed` DESC ";

    $my_num_rows = 20;

    include 'my_pagination_search.php';

    $count_res=$link->query($query_one)->num_rows;
?>
        ...
        <div class="page-container">
            ...
            <div class="main-content" style="padding: 20px;">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="title-1 m-b-25"><i class="fa fa-folder-open"></i> <?php echo $title; ?> </h2>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <strong class="card-title mb-3" style="text-transform: uppercase;"><span style="color: blue;"><?= 0 + $count_res; ?></span> requests</strong>
                                </div>
                                <div class="card-body">
