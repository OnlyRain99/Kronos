...
$fields = array('HRID' ,'Name', 'Account', 'Shift Date', 'LogIn Time', 'BreakOut Time', 'BreakIn Time', 'LogOut Time', 'Schedule Start','Schedule End','Schedule Hours','Actual Hours','Break Hours');
...
function tblcntt($mod, $id, $name, $accnm, $date, $schin, $schout, $login, $logout, $bout, $bin, $bh){
$ah=0; $th=0;
...
$lineData = array($id, $name, $accnm, date("m/d/Y", strtotime($date)), chktime($login), chktime($bout), chktime($bin), chktime($logout), $schin, $schout, $th, gettwh($ah), gettwh($bh));
...
for($i1=0;$i1<count($usrarr);$i1++){
	...
	while($tmsrow=$tmsht->fetch_array()){
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			$lineData = tblcntt($cnttruedate[4], $tmsrow['gy_emp_code'], $userarr[0][$i1], $userarr[2][0], $cntdate, chktime($cnttruedate[2]), chktime($cnttruedate[3]), "", "", "", "", 0);
			fputcsv($fp, $lineData);
...
if(date("Y-m-d", strtotime($truedate[0]))>=date('Y-m-d', strtotime($fdate)) && date("Y-m-d", strtotime($truedate[0]))<=date('Y-m-d', strtotime($tdate))){
		...
		$lineData = tblcntt($truedate[4], $tmsrow['gy_emp_code'], $userarr[0][$i1], $userarr[2][$i1], $truedate[0], chktime($truedate[2]), chktime($truedate[3]), $tmsrow['gy_tracker_login'], $tmsrow['gy_tracker_logout'], $tmsrow['gy_tracker_breakout'], $tmsrow['gy_tracker_breakin'], $tmsrow['gy_tracker_bh']);
		fputcsv($fp, $lineData);
}
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
	while($cntdate<=date("Y-m-d", strtotime($tdate))){
		...
		$lineData = tblcntt($truedate[4], $usrarr[$i1], $userarr[0][$i1], $userarr[2][0], $cntdate, chktime($truedate[2]), chktime($truedate[3]), "", "", "", "", 0);
		fputcsv($fp, $lineData);
		$cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
	}
}
