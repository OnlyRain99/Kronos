...
$ucsql=$link->query("SELECT `gy_user`.`gy_user_code` as `gyucode`, `gy_user`.`gy_full_name` as `gyflname` From `gy_employee` LEFT JOIN `gy_user` ON `gy_employee`.`gy_emp_code`=`gy_user`.`gy_user_code` Where (`gy_user`.`gy_user_code`='$user_code' OR `gy_employee`.`gy_acc_id`=11 OR `gy_user`.`gy_user_type`=3) AND `gy_user`.`gy_user_status`=0 ");
$i=0;
echo '[';
