...
function tblcntt($mode, $empcode, $email, $fname, $flname, $mname, $dept, $date, $login, $logout, $si, $so, $chatc, $emailc, $phonec){
    if($mode==0){
...
<tr>
    ...
    <th style="padding: 5px;" class="text-nowrap text-center"><?php echo $flname; ?></th>
    <th style="padding: 5px;" class="text-center"><?php echo $dept; ?></th>
    <th style="padding: 5px;" class="text-center"><?php echo $date; ?></th>
    <th style="padding: 5px;" class="text-nowrap text-center"><?php echo chktime($login); ?></th>
    <th style="padding: 5px;" class="text-nowrap text-center"><?php echo chktime($logout); ?></th>
    <th style="padding: 5px;" class="text-nowrap text-center"><?php echo chktime($si); ?></th>
    <th style="padding: 5px;" class="text-nowrap text-center"><?php echo chktime($so); ?></th>
...
if($fdate == "" && $tdate != ""){ $sqlseq = "AND `gy_tracker_date` >='".date('Y-m-d', strtotime($tdate.' -1 day'))."' AND `gy_tracker_date` <= '".date('Y-m-d', strtotime($tdate.' +1 day'))."'"; }
    ...
    $tmsht=$link->query("SELECT `gy_tracker_status`,`gy_tracker_date`,`gy_tracker_login`,`gy_tracker_logout`,`gy_tracker_breakout`,`gy_tracker_breakin` From `gy_tracker` Where `gy_emp_code`='".$gyempar[$i1]."' AND `gy_tracker_request`!='reject' ".$sqlseq." Order By `gy_tracker_date` ASC");
        ...
        while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
            ...
            tblcntt($thismode, $gyempar[$i1], $gyemparr[$i1][0], $gyemparr[$i1][1], $gyemparr[$i1][2], $gyemparr[$i1][3], $gyemparr[$i1][4], date("m/d/Y", strtotime($cntdate)), "No Log", "No Log", $cnttruedate[2], $cnttruedate[3], 0, 0, 0);
...
if(date("Y-m-d", strtotime($truedate[0]))>=date('Y-m-d', strtotime($fdate)) && date("Y-m-d", strtotime($truedate[0]))<=date('Y-m-d', strtotime($tdate))){
    ...
    tblcntt($thismode, $gyempar[$i1], $gyemparr[$i1][0], $gyemparr[$i1][1], $gyemparr[$i1][2], $gyemparr[$i1][3], $gyemparr[$i1][4], date("m/d/Y", strtotime($truedate[0])), $tsrow['gy_tracker_login'], $tsrow['gy_tracker_logout'], $truedate[2], $truedate[3], $channelepc[0], $channelepc[1], $channelepc[2]);
}
    $cntdate = date('Y-m-d', strtotime($cntdate.' +1 day'));
        }
