<?php 
    ...
    if ($fil == "all") {
        $filter_title = "All";
        $filter = "`gy_escalate`.`gy_esc_status`!='0'";
    }else if ($fil == 1) {
        $filter_title = "Approved";
        $filter = "`gy_escalate`.`gy_esc_status`='1'";
    }else if ($fil == 2) {
        $filter_title = "Denied";
        $filter = "`gy_escalate`.`gy_esc_status`='2'";
    }else{
        $filter_title = "unknown";
        $filter = "`gy_escalate`.`gy_esc_status`!='0'";
    }

    $title = "Escalate Request History: ".$finaldate." - ".$filter_title;

    $query_one = "SELECT * From `gy_escalate` LEFT JOIN `gy_user` ON `gy_escalate`.`gy_usercode`=`gy_user`.`gy_user_code` Where ".$filter." AND `gy_user`.`gy_user_type`<=5 AND `gy_escalate`.`gy_esc_date` BETWEEN '$f' AND '$t' Order By `gy_escalate`.`gy_esc_date` ASC";
    $query_two = "SELECT COUNT(`gy_esc_id`) From `gy_escalate` LEFT JOIN `gy_user` ON `gy_escalate`.`gy_usercode`=`gy_user`.`gy_user_code` Where ".$filter." AND `gy_user`.`gy_user_type`<=5 AND `gy_escalate`.`gy_esc_date` BETWEEN '$f' AND '$t'";
    $query_three = "SELECT * From `gy_escalate` LEFT JOIN `gy_user` ON `gy_escalate`.`gy_usercode`=`gy_user`.`gy_user_code` Where ".$filter." AND `gy_user`.`gy_user_type`<=5 AND `gy_escalate`.`gy_esc_date` BETWEEN '$f' AND '$t' Order By `gy_escalate`.`gy_esc_date` ASC ";
                                                ...
                                                <table class="table table-bordered" style="font-family: 'Calibri'; font-size: 14px;">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr class="<?= $mybg; ?>">
                                                            ...
                                                            <td style="padding: 0px;" class="text-center"><a href="view_escalate?cd=<?= $escrow['gy_esc_id']; ?>" class="btn btn-warning btn-sm btn-block" onclick="window.open(this.href, 'mywin',
