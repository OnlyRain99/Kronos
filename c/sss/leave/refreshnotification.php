...
if($type==0){
    ...
    $leavecount = get_leave_pending_requests(getall_leave_level3($user_id))+get_leave_pending_requests($user_id); ?>
    <i class="fa-solid fa-calendar-day faa-passing faa-reverse"></i> My Leave Calendar <?php if($leavecount>0){ ?><span class="badge badge-danger" title="<?php echo $leavecount." LOA pending request"; ?>"><?php echo $leavecount; ?></span><?php }
}else if($type==2){
    $leavecount = get_leave_pending_requests(getall_leave_level3($user_id))+get_leave_pending_requests($user_id); ?>
