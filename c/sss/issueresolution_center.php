...
<body>
    ...
    if(date("d")<=3){ $datestrt = date("Y-m-16", strtotime("-1 Month")); }
        ...
        $today = date("Y-m-d");
            ...
            $i = 0;
                $schesql = $link->query("SELECT * From `gy_schedule_escalate` LEFT JOIN `gy_user` ON `gy_schedule_escalate`.`gy_emp_code`=`gy_user`.`gy_user_code` Where `gy_schedule_escalate`.`gy_req_status`='0' AND `gy_user`.`gy_user_type`<=5 AND `gy_schedule_escalate`.`gy_sched_day`>='$datestrt' ORDER BY `gy_schedule_escalate`.`gy_sched_esc_id` desc");
                while ($scherow=$schesql->fetch_array()){
                    $own = 0; if($scherow['gy_emp_code']==$user_code){ $own = 1; }
                    cardcontent($i, 0, $own, $scherow['gy_req_date'], $scherow['gy_emp_fullname'], $scherow['gy_req_status'], $scherow['gy_req_deny'], $scherow['gy_req_by'], $scherow['gy_req_to'], $scherow['gy_sup'], $scherow['gy_sched_mode'], $scherow['gy_sched_day'], $scherow['gy_sched_login'], $scherow['gy_sched_breakout'], $scherow['gy_sched_breakin'], $scherow['gy_sched_logout'], $scherow['gy_req_reason'], $scherow['msg_usercode'], $scherow['gy_sched_esc_id'], $scherow['gy_publish'], $user_code); $i++; }

                $logssql = $link->query("SELECT `gy_tracker`.`gy_emp_code`as`gycode`,`gy_escalate`.`gy_esc_date`as`gyescdate`,`gy_tracker`.`gy_emp_fullname`as`gyflname`,`gy_escalate`.`gy_esc_status`as`gyescstatus`,`gy_escalate`.`gy_esc_deny`as`gyescdeny`,`gy_escalate`.`gy_esc_by`as`gyescby`,`gy_escalate`.`gy_esc_to`as`gyescto`,`gy_escalate`.`gy_sup`as`gysup`,`gy_escalate`.`gy_esc_type`as`gyesctype`,`gy_escalate`.`gy_tracker_date`as`gytrckdate`,`gy_escalate`.`gy_tracker_login`as`gytrcklin`,`gy_escalate`.`gy_tracker_breakout`as`gybout`,`gy_escalate`.`gy_tracker_breakin`as`gybin`,`gy_escalate`.`gy_tracker_logout`as`gytrckout`,`gy_escalate`.`gy_esc_reason`as`gyescrsn`,`gy_escalate`.`msg_usercode`as`gymsgu`,`gy_escalate`.`gy_esc_id`as`gyescid`,`gy_escalate`.`gy_publish`as`gypblsh` From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` LEFT JOIN `gy_user` ON `gy_escalate`.`gy_usercode`=`gy_user`.`gy_user_code` Where `gy_escalate`.`gy_esc_status`='0' AND `gy_user`.`gy_user_type`<=5 AND `gy_escalate`.`gy_tracker_date`>='$datestrt' ORDER BY `gy_escalate`.`gy_esc_id` desc");
                while ($logrow=$logssql->fetch_array()){
                    $own = 0; if($logrow['gycode']==$user_code){ $own = 1; }
                    cardcontent($i, 1, $own, $logrow['gyescdate'], $logrow['gyflname'], $logrow['gyescstatus'], $logrow['gyescdeny'], $logrow['gyescby'], $logrow['gyescto'], $logrow['gysup'], $logrow['gyesctype'], $logrow['gytrckdate'], $logrow['gytrcklin'], $logrow['gybout'], $logrow['gybin'], $logrow['gytrckout'], $logrow['gyescrsn'], $logrow['gymsgu'], $logrow['gyescid'], $logrow['gypblsh'], $user_code); $i++; }

                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_user_id`='$user_id' OR  `gy_employee`.`gy_emp_supervisor`='$user_id') AND `gy_leave`.`gy_leave_status`=0 ");
                while($glsrow=$ctlsql->fetch_array()){
                    $own='Team';
                    if($glsrow['gy_user_id']==$user_id){ $own='My';  }
                    cardloacont($glsrow['gy_leave_id'], $glsrow['gy_leave_status'], $own, $glsrow['gy_emp_fullname'], $glsrow['gy_leave_filed'], get_leave_type($glsrow['gy_leave_type']), $glsrow['gy_leave_date_from'], $glsrow['gy_leave_reason'], $glsrow['gy_leave_date_approved'], getuserfullname($glsrow['gy_leave_approver']), $glsrow['gy_leave_remarks']);
                }
                ...
                $logssql = $link->query("SELECT `gy_tracker`.`gy_emp_code`as`gycode`,`gy_escalate`.`gy_esc_date`as`gyescdate`,`gy_tracker`.`gy_emp_fullname`as`gyflname`,`gy_escalate`.`gy_esc_status`as`gyescstatus`,`gy_escalate`.`gy_esc_deny`as`gyescdeny`,`gy_escalate`.`gy_esc_by`as`gyescby`,`gy_escalate`.`gy_esc_to`as`gyescto`,`gy_escalate`.`gy_sup`as`gysup`,`gy_escalate`.`gy_esc_type`as`gyesctype`,`gy_escalate`.`gy_tracker_date`as`gytrckdate`,`gy_escalate`.`gy_tracker_login`as`gytrcklin`,`gy_escalate`.`gy_tracker_breakout`as`gybout`,`gy_escalate`.`gy_tracker_breakin`as`gybin`,`gy_escalate`.`gy_tracker_logout`as`gytrckout`,`gy_escalate`.`gy_esc_reason`as`gyescrsn`,`gy_escalate`.`msg_usercode`as`gymsgu`,`gy_escalate`.`gy_esc_id`as`gyescid`,`gy_escalate`.`gy_publish`as`gypblsh` From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` Where `gy_esc_status`='1' AND `gy_escalate`.`gy_tracker_date`>='$today' AND (`gy_tracker`.`gy_emp_code`='$user_code' OR `gy_escalate`.`gy_esc_by`='$user_id' OR `gy_escalate`.`gy_esc_to`='$user_id' OR `gy_escalate`.`gy_sup`='$user_id') ORDER BY `gy_esc_id` desc");
                while ($logrow=$logssql->fetch_array()){
                    $own = 0; if($logrow['gycode']==$user_code){ $own = 1; }
                    cardcontent($i, 1, $own, $logrow['gyescdate'], $logrow['gyflname'], $logrow['gyescstatus'], $logrow['gyescdeny'], $logrow['gyescby'], $logrow['gyescto'], $logrow['gysup'], $logrow['gyesctype'], $logrow['gytrckdate'], $logrow['gytrcklin'], $logrow['gybout'], $logrow['gybin'], $logrow['gytrckout'], $logrow['gyescrsn'], $logrow['gymsgu'], $logrow['gyescid'], $logrow['gypblsh'], $user_code); $i++; }
            ...
            if(date("d")>15){ $fdotco = date("Y-m-01"); }else{ $fdotco = date("Y-m-15", strtotime("-1 month")); }
                ...
                while ($logrow=$logssql->fetch_array()){
                    ...
                    cardcontent($i, 1, $own, $logrow['gy_esc_date'], $logrow['gy_emp_fullname'], $logrow['gy_esc_status'], $logrow['gy_esc_deny'], $logrow['gy_esc_by'], $logrow['gy_esc_to'], $logrow['gy_sup'], $logrow['gy_esc_type'], $logrow['gy_tracker_date'], $logrow['gy_tracker_login'], $logrow['gy_tracker_breakout'], $logrow['gy_tracker_breakin'], $logrow['gy_tracker_logout'], $logrow['gy_esc_reason'], $logrow['msg_usercode'], $logrow['gy_esc_id'], $logrow['gy_publish'], $user_code); $i++; }

                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_user_id`='$user_id' OR  `gy_employee`.`gy_emp_supervisor`='$user_id') AND (`gy_leave`.`gy_leave_date_from`>='$today' OR `gy_leave`.`gy_leave_date_approved`>='$today') AND `gy_leave`.`gy_leave_status`=1 ");
                while($glsrow=$ctlsql->fetch_array()){
                    $own='Team';
                    if($glsrow['gy_user_id']==$user_id){ $own='My';  }
                    cardloacont($glsrow['gy_leave_id'], $glsrow['gy_leave_status'], $own, $glsrow['gy_emp_fullname'], $glsrow['gy_leave_filed'], get_leave_type($glsrow['gy_leave_type']), $glsrow['gy_leave_date_from'], $glsrow['gy_leave_reason'], $glsrow['gy_leave_date_approved'], getuserfullname($glsrow['gy_leave_approver']), $glsrow['gy_leave_remarks']);
                ...
                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_user_id`='$user_id' OR  `gy_employee`.`gy_emp_supervisor`='$user_id') AND (`gy_leave`.`gy_leave_date_from`>='$today' OR `gy_leave`.`gy_leave_date_approved`>='$today') AND `gy_leave`.`gy_leave_status`=2 ");
                while($glsrow=$ctlsql->fetch_array()){
                    $own='Team';
                    if($glsrow['gy_user_id']==$user_id){ $own='My';  }
                    cardloacont($glsrow['gy_leave_id'], $glsrow['gy_leave_status'], $own, $glsrow['gy_emp_fullname'], $glsrow['gy_leave_filed'], get_leave_type($glsrow['gy_leave_type']), $glsrow['gy_leave_date_from'], $glsrow['gy_leave_reason'], $glsrow['gy_leave_date_approved'], getuserfullname($glsrow['gy_leave_approver']), $glsrow['gy_leave_remarks']);
            ...
            function cardloacont($cardn, $reqstatus, $own, $fname, $fyldte, $loanm, $loadate, $reason, $rspdate, $rspby, $rmrks){
                ...
                <?php } function cardcontent($cardn, $esct, $own, $reqdate, $fname, $reqstatus, $reqdeny, $reqby, $reqto, $sup, $schmode, $schday, $schli, $schbo, $schbi, $schlo, $reqreson, $msguc, $rfid, $publish, $ucode){
                ...
                <div class="card hover-overlay shadow rounded">
                    ...
                    <div class="card-footer <?php echo $mask; ?>">
                        ...
                        <div class="pull-right">
                            ...
                            <btn class="btn-outline-dark btn-sm" title="Check on My Team?" ><i class="fas fa-paper-plane"></i></btn>
                        </div>
                    </div>
