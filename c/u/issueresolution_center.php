...
<body>
    ...
    if(date("d")<=3){ $datestrt = date("Y-m-16", strtotime("-1 Month")); }
        ...
        $today = date("Y-m-d");
            ...
            $i = 0;
                $schesql = $link->query("SELECT * From `gy_schedule_escalate` Where `gy_emp_code`='$user_code' AND `gy_req_status`='0' AND `gy_sched_day`>='$datestrt' ORDER BY `gy_sched_esc_id` desc");
                while ($scherow=$schesql->fetch_array()){ cardcontent($i, 0, $scherow['gy_req_date'], $scherow['gy_emp_code'], $scherow['gy_req_status'], $scherow['gy_req_deny'], $scherow['gy_req_by'], $scherow['gy_req_to'], $scherow['gy_sup'], $scherow['gy_sched_mode'], $scherow['gy_sched_day'], $scherow['gy_sched_login'], $scherow['gy_sched_breakout'], $scherow['gy_sched_breakin'], $scherow['gy_sched_logout'], $scherow['gy_req_reason']); $i++; }

                $logssql = $link->query("SELECT * From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` Where `gy_esc_status`='0' AND `gy_tracker`.`gy_emp_code`='$user_code' AND `gy_escalate`.`gy_tracker_date`>='$datestrt' ORDER BY `gy_esc_id` desc");
                while ($logrow=$logssql->fetch_array()){ cardcontent($i, 1, $logrow['gy_esc_date'], $logrow['gy_emp_code'], $logrow['gy_esc_status'], $logrow['gy_esc_deny'], $logrow['gy_esc_by'], $logrow['gy_esc_to'], $logrow['gy_sup'], $logrow['gy_esc_type'], $logrow['gy_tracker_date'], $logrow['gy_tracker_login'], $logrow['gy_tracker_breakout'], $logrow['gy_tracker_breakin'], $logrow['gy_tracker_logout'], $logrow['gy_esc_reason']); $i++; }
                ...
                $schesql = $link->query("SELECT * From `gy_schedule_escalate` Where `gy_emp_code`='$user_code' AND `gy_req_status`='1' AND `gy_sched_day`>='$today' ORDER BY `gy_sched_esc_id` desc");
                while ($scherow=$schesql->fetch_array()){ cardcontent($i, 0, $scherow['gy_req_date'], $scherow['gy_emp_code'], $scherow['gy_req_status'], $scherow['gy_req_deny'], $scherow['gy_req_by'], $scherow['gy_req_to'], $scherow['gy_sup'], $scherow['gy_sched_mode'], $scherow['gy_sched_day'], $scherow['gy_sched_login'], $scherow['gy_sched_breakout'], $scherow['gy_sched_breakin'], $scherow['gy_sched_logout'], $scherow['gy_req_reason']); $i++; }

                $logssql = $link->query("SELECT * From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` Where `gy_esc_status`='1' AND `gy_escalate`.`gy_tracker_date`>='$today' AND `gy_tracker`.`gy_emp_code`='$user_code' ORDER BY `gy_esc_id` desc");
                while ($logrow=$logssql->fetch_array()){ cardcontent($i, 1, $logrow['gy_esc_date'], $logrow['gy_emp_code'], $logrow['gy_esc_status'], $logrow['gy_esc_deny'], $logrow['gy_esc_by'], $logrow['gy_esc_to'], $logrow['gy_sup'], $logrow['gy_esc_type'], $logrow['gy_tracker_date'], $logrow['gy_tracker_login'], $logrow['gy_tracker_breakout'], $logrow['gy_tracker_breakin'], $logrow['gy_tracker_logout'], $logrow['gy_esc_reason']); $i++; }
                ...
                $schesql = $link->query("SELECT * From `gy_schedule_escalate` Where `gy_emp_code`='$user_code' AND `gy_req_status`='2' AND `gy_sched_day`>='$today' ORDER BY `gy_sched_esc_id` desc");
                while ($scherow=$schesql->fetch_array()){ cardcontent($i, 0, $scherow['gy_req_date'], $scherow['gy_emp_code'], $scherow['gy_req_status'], $scherow['gy_req_deny'], $scherow['gy_req_by'], $scherow['gy_req_to'], $scherow['gy_sup'], $scherow['gy_sched_mode'], $scherow['gy_sched_day'], $scherow['gy_sched_login'], $scherow['gy_sched_breakout'], $scherow['gy_sched_breakin'], $scherow['gy_sched_logout'], $scherow['gy_req_reason']); $i++; }

                $logssql = $link->query("SELECT * From `gy_escalate` LEFT JOIN `gy_tracker` ON `gy_escalate`.`gy_tracker_id`=`gy_tracker`.`gy_tracker_id` Where `gy_esc_status`='2' AND `gy_escalate`.`gy_tracker_date`>='$today' AND `gy_tracker`.`gy_emp_code`='$user_code' ORDER BY `gy_esc_id` desc");
                while ($logrow=$logssql->fetch_array()){ cardcontent($i, 1, $logrow['gy_esc_date'], $logrow['gy_emp_code'], $logrow['gy_esc_status'], $logrow['gy_esc_deny'], $logrow['gy_esc_by'], $logrow['gy_esc_to'], $logrow['gy_sup'], $logrow['gy_esc_type'], $logrow['gy_tracker_date'], $logrow['gy_tracker_login'], $logrow['gy_tracker_breakout'], $logrow['gy_tracker_breakin'], $logrow['gy_tracker_logout'], $logrow['gy_esc_reason']); $i++; }
            ...
            function cardcontent($cardn, $esct, $reqdate, $empid, $reqstatus, $reqdeny, $reqby, $reqto, $sup, $schmode, $schday, $schli, $schbo, $schbi, $schlo, $reqreson){
                    ...
                    if($reqstatus==0){ $color="warning"; $ctitle="<i class='fa-solid fa-clock-rotate-left faa-wrench'></i> Pending Escalation"; $mask="maskp"; $perc="50%"; $finalicon='<i class="fa fa-question"></i>'; }
                ...
                <div class="card hover-overlay shadow rounded">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <div class="position-relative m-4">
                            ...
                            <button type="button" class="position-absolute top-0 start-0 translate-middle btn btn-sm <?php echo "btn-".$color; ?> rounded-pill" style="width: 2rem; height:2rem;" data-bs-toggle="popover" data-bs-placement="left" title="<?php echo $escname; ?>" data-bs-content="Requested By <?php echo getuserfullname($reqby); ?>">
                                <i class='far fa-user'></i>
                            </button>
                            <?php if($reqstatus==0){ ?>
                            <button type="button" class="position-absolute top-0 start-50 translate-middle btn btn-sm rounded-pill" data-bs-toggle="popover" data-bs-placement="top" data-bs-content="Waiting for Response">
                                <div class="spinner-grow text-warning" role="status"></div>
                            </button>
                            <?php } ?>
                    ...
                    <div class="card-footer <?php echo $mask; ?>">
                        ...
                        <div class="pull-right">
                            ...
                            <btn class="btn-outline-dark btn-sm" title="Check on My Team?" <?php echo "onclick=cnfrmmyteam($cardn)"; ?> ><i class="fas fa-paper-plane"></i></btn>
                        </div>
                    </div>
                    <form method="post" id="tomyteam_<?php echo $cardn; ?>" action="mydailylogs" enctype="multipart/form-data">
