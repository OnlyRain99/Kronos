...
function matchsched($dbemp, $dblogin, $dbbreo, $dbbrei, $dblogo){
	...
	if(mysqli_num_rows($empsch) > 0){
		while ($scrow=$empsch->fetch_array()) {
			...
			//$arrsched[1] =$scrow['gy_sched_day']." ".convert24to0($scrow['gy_sched_login'])." - ".date("Y-m-d H:i:s", $schedlout);
			break; }
		}
	}
    $link->close();
	...
	if(mysqli_num_rows($empsch) > 0){
		while ($scrow=$empsch->fetch_array()) {
			...
			if($dblogin > $schedin){ //$ut[0] = getmindif($schedin, $dblogin, "in"); 
			    $ut[0] = getdtrwh($schedin, $dblogin);
			}
			if($dblogout < $schedout && $dblogout != "0000-00-00 00:00:00"){ //$ut[1] = getmindif($dblogout, $schedout, "out"); 
			    $ut[1] = getdtrwh($dblogout, $schedout);
			}

			//if($dblogin < $schedin){ $ut[2] = getmindif($dblogin, $schedin, "in"); }
			//if($dblogout > $schedout && $dblogout != "0000-00-00 00:00:00"){ $ut[2] += getmindif($schedout, $dblogout, "out"); }
...
function getaot($trkid, $trkcd, $lilo, $scdate, $scin, $scout){
$ot = 0;
$requestdate="";

	$escsql=$link->query("SELECT `gy_tracker_login`, `gy_tracker_breakout`, `gy_tracker_breakin`, `gy_tracker_logout`, `gy_esc_date` FROM `gy_escalate` WHERE `gy_esc_type`=6 AND `gy_esc_status`=1 AND `gy_tracker_id`='$trkid' ORDER BY `gy_esc_id` desc Limit 1");
	while($escrow=$escsql->fetch_array()){
		if(strtotime($escrow['gy_tracker_login'])<$sftin && getdtrwh($escrow['gy_tracker_login'], date("Y-m-d H:i:s", $sftin))>=0.5){ $ot+=getdtrwh($escrow['gy_tracker_login'], date("Y-m-d H:i:s", $sftin)); }
		if(strtotime($escrow['gy_tracker_logout'])>$sftout && getdtrwh(date("Y-m-d H:i:s", $sftout), $escrow['gy_tracker_logout'])>=0.5){ $ot+=getdtrwh(date("Y-m-d H:i:s", $sftout), $escrow['gy_tracker_logout']); }
		$requestdate=$escrow['gy_esc_date'];
	}
	$schsql=$link->query("SELECT `gy_tracker_login`,`gy_tracker_logout`,`gy_sched_login`,`gy_sched_logout`,`gy_sched_day`,`gy_req_date` FROM `gy_schedule_escalate` WHERE `gy_req_status`=1 AND `gy_sched_mode`=2 AND `gy_tracker_login`!='0000-00-00 00:00:00' AND `gy_tracker_logout`!='0000-00-00 00:00:00' AND `gy_sched_esc_code`=$trkcd ORDER BY `gy_sched_esc_id` desc Limit 1");
    while($schrow=$schsql->fetch_array()){
		if(strtotime($schrow['gy_req_date'])>strtotime($requestdate)){
			...
			if(strtotime($schrow['gy_tracker_login'])<strtotime($tmpin) && getdtrwh($schrow['gy_tracker_login'], $tmpin)>=0.5){ $ot+=getdtrwh($schrow['gy_tracker_login'], $tmpin); }
			if(strtotime($schrow['gy_tracker_logout'])>strtotime($tmpout) && getdtrwh($tmpout, $schrow['gy_tracker_logout'])>=0.5){ $ot+=getdtrwh($tmpout, $schrow['gy_tracker_logout']); }
		}
	}
    $link->close();
    return $ot;
...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='$user_code' ".$sqlseq." Order By `gy_tracker_date` ASC");
	while ($tsrow=$tmsht->fetch_array()) {
		...
		while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
			...
			$thismode = getmode($tsrow['gy_emp_code'], date("Y-m-d", strtotime($truedate[0])), 1);
			
			if ($tsrow['gy_tracker_status'] == 1) {
                if ($thismode == 1 || $thismode == 2) {
                	$undertime = getundert($tsrow['gy_emp_code'], $truedate[0], $lilo[0], $lilo[3], $tsrow['gy_tracker_id'], $tsrow['gy_tracker_code']);
                	//$ot = getaot($tsrow['gy_tracker_id'], $tsrow['gy_tracker_code'], $truedate[0]);
                }
            }
...
<td style="padding: 5px;" class="text-center <?php echo $txtcolor[9]; ?>"><?php echo round($theath, 2); //if($theath>0){$xtheath=explode(".",$theath,2); if($xtheath[1]){echo round($xtheath[1]/60,2)+$xtheath[0];}else{echo$xtheath[0];}} ?></td>
