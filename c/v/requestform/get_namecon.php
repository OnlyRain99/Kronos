...
if($schrow['msg_usercode']!=""){ $strsql = $schrow['msg_usercode'].",".$user_code; 
...
if($escrow['msg_usercode']!=""){ $strsql = $escrow['msg_usercode'].",".$user_code;
...
$ucsql=$link->query("SELECT `gy_user_code`,`gy_full_name` From `gy_user` Where (`gy_user_code`='$user_code' OR `gy_user_type` > 3) AND `gy_user_status`=0 ");
$i=0;
echo '[';
while($ucrow=$ucsql->fetch_array()){
if($i>0){ echo ','; }
echo '{ "ucode":"'.$ucrow['gy_user_code'].'", "fullname":"'.$ucrow['gy_full_name'].'" }';
$i++;
}
echo ']';
$link->close();
