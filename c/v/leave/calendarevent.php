<?php
    ...
    $levsql=$link->query("SELECT * From `gy_leave_available` LEFT JOIN `gy_accounts` on `gy_leave_available`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` WHERE `gy_leave_available`.`gy_acc_id`='$myaccount' OR `gy_accounts`.`gy_dept_id`=2 order by `gy_leave_available`.`gy_leave_avail_date` asc");
...
$ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE `gy_leave`.`gy_user_id`!='$user_id' AND (`gy_employee`.`gy_emp_supervisor`='$user_id' OR `gy_leave`.`gy_acc_id`='$myaccount') AND `gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']);
