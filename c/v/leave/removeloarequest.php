...
$attachment=$link->query("SELECT `gy_leave_attachment`,`gy_leave_status`,`gy_acc_id`,`gy_leave_date_from`,`gy_leave_day`,`gy_user_id`,`gy_publish` From `gy_leave` Where `gy_leave_id`='$lid' AND `gy_publish`=0");
...
if($attach['gy_leave_status']==1){
    ...
    if($ldatefrm>=$scutoff){
    $deletedata=$link->query("DELETE FROM `gy_leave` Where `gy_leave_id`='$lid' AND `gy_user_id`='$user_id' AND `gy_leave_status`=1 AND `gy_publish`=0");
    if($deletedata){
