...
if($sftin>$sftout){ $sftout = strtotime(date("Y-m-d", strtotime($scdate))." ".$scout."+1 day"); } //sched lilo
...
$requestdate="";
    ...
    $escsql=$link->query("SELECT `gy_tracker_login`, `gy_tracker_breakout`, `gy_tracker_breakin`, `gy_tracker_logout`, `gy_esc_date` FROM `gy_escalate` WHERE `gy_esc_type`=6 AND `gy_esc_status`=1 AND `gy_tracker_id`=$trkid ORDER BY `gy_esc_id` desc Limit 1");
        while($escrow=$escsql->fetch_array()){
            ...
            else if(strtotime($escrow['gy_tracker_login'])<$sftin && getdtrwh($escrow['gy_tracker_login'], date("Y-m-d H:i:s", $sftin))>=0.5){ $lilo[0]=$escrow['gy_tracker_login']; $lilo[4]="Approved early OT from ".date("h:iA", strtotime($lilo[0]))." ".$scdate; }
    ...
    $schsql=$link->query("SELECT `gy_tracker_login`,`gy_tracker_logout`,`gy_sched_login`,`gy_sched_logout`,`gy_sched_day`,`gy_req_date` FROM `gy_schedule_escalate` WHERE `gy_req_status`=1 AND `gy_sched_mode`=2 AND `gy_tracker_login`!='0000-00-00 00:00:00' AND `gy_tracker_logout`!='0000-00-00 00:00:00' AND `gy_sched_esc_code`=$trkcd ORDER BY `gy_sched_esc_id` desc Limit 1");
        while($schrow=$schsql->fetch_array()){
          if($requestdate=="" || strtotime($schrow['gy_req_date'])>strtotime($requestdate)){
...
$dayarr[1] = $holdate;
$sftin = strtotime($holdate." ".convert24to0($shiftin));
$sftout = strtotime($holdate." ".convert24to0($shiftout));

if(strtotime(convert24to0($shiftin))>strtotime(convert24to0($shiftout))){ $sftout = strtotime($holdate." ".convert24to0($shiftout)." +1 day"); }
$slin=strtotime($lin);
...
//if($holdate=="2023-04-28"){ echo $sftin."-".$sftout." ";}
...
if($mode==1){ $reg=0; $ot=0; $nd=4; $ndot=4; for($day=0;$day<count($dayarr);$day++){
    if($day==0){
      for($i3=0;$i3<count($d0dix);$i3++){
        if($slin<=$ymidn && $slin>=$ynstart && $sbout>=$ymidn){ $holarr[$d0dix[$i3]][$ndot]+=($ymidn-$slin); }//early NDOT echo"y1";
        else if($sbout<=$ymidn && $slin>=$ynstart){ $holarr[$d0dix[$i3]][$ndot]+=($sbout-$slin);}//early NDOT echo"y2";
        else if($sbout>$ymidn && $slin<$ynstart){ $holarr[$d0dix[$i3]][$ndot]+=($ymidn-$ynstart);}//early NDOT echo"y3";
        else if($slin<$ynstart && $sbout>=$ynstart && $sbout<=$ymidn){ $holarr[$d0dix[$i3]][$ndot]+=($sbout-$ynstart);}//early NDOT echo"y4";

        if($sbin<=$ymidn && $sbin>=$ynstart && $slout>$ymidn){ $holarr[$d0dix[$i3]][$ndot]+=($ymidn-$sbin);}//early NDOT echo"y5";
        else if($sbin<=$ynstart){ $holarr[$d0dix[$i3]][$ndot]+=($ymidn-$ynstart);}//early NDOTecho"y6";

        if($slin<$ynstart && $sbout>=$ynstart){ $holarr[$d0dix[$i3]][$ot]+=($ynstart-$slin);}//early OTecho"y7";
        else if($slin<$ynstart && $sbout<=$ynstart){ $holarr[$d0dix[$i3]][$ot]+=($sbout-$slin);}//early OT echo"y8";
        else if($sbin<$ynstart){ $holarr[$d0dix[$i3]][$ot]+=($ynstart-$sbin);}//early OT echo"y9";
      }
    }else if($day==1){
      for($i3=0;$i3<count($d1dix);$i3++){
       //new morning ND
       if($sftout>$ymidn && $sftin<$ynend){
        if($sftin>=$ymidn && $sbout<=$ynend){ $holarr[$d1dix[$i3]][$nd]+=($sbout-$sftin);} //ND BO echo"a1";
        else if($sftin<$ymidn && $sbout<=$ynend && $sbout>$ymidn){ $holarr[$d1dix[$i3]][$nd]+=($sbout-$ymidn);}//echo"a2";
        else if($sftin>=$ymidn && $sftin<$ynend && $sbout>=$ynend){ $holarr[$d1dix[$i3]][$nd]+=($ynend-$sftin);}//echo"a3";
        else if($sftin>$ymidn && $sbout>$ynend){ $holarr[$d1dix[$i3]][$nd]+=($ynend-$ymidn);}//echo"a4";

        if($sbin>=$ymidn && $sftout<=$ynend){ $holarr[$d1dix[$i3]][$nd]+=($sftout-$sbin);} //ND BI echo"a5";
        else if($sbin>=$ymidn && $sbin<$ynend && $sftout>$ynend){ $holarr[$d1dix[$i3]][$nd]+=($ynend-$sbin);}//echo"a6";
        else if($sbin<=$ymidn && $sftout>$ymidn && $sftout<=$ynend){ $holarr[$d1dix[$i3]][$nd]+=($sftout-$ymidn);}//echo"a7";
        else if($sbin<$ymidn && $sftout>$ynend){ $holarr[$d1dix[$i3]][$nd]+=($ynend-$ymidn);}//echo"a8";
       }
       //new night ND
       if($sftin<$midn && $sftout>$nstart){
        if($sftin>=$nstart && $sbout<=$midn){ $holarr[$d1dix[$i3]][$nd]+=($sbout-$sftin);}//ND BO echo"a9";
        else if($sftin>=$nstart && $sftin<$midn && $sbout>$midn){ $holarr[$d1dix[$i3]][$nd]+=($midn-$sftin);}//echo"a10";
        else if($sftin<$nstart && $sbout>$nstart && $sbout<=$midn){ $holarr[$d1dix[$i3]][$nd]+=($sbout-$nstart);}//echo"a11";
        else if($sftin<$nstart && $sbout>$midn){ $holarr[$d1dix[$i3]][$nd]+=($midn-$nstart);}//echo"a12";

        if($sbin>=$nstart && $sftout<=$midn){ $holarr[$d1dix[$i3]][$nd]+=($sftout-$sbin);}//ND BI/echo"a13";
        else if($sbin>=$nstart && $sbin<$midn && $sftout>$midn){ $holarr[$d1dix[$i3]][$nd]+=($midn-$sbin);}//echo"a14";
        else if($sbin<$nstart && $sftout>$nstart && $sftout<=$midn){ $holarr[$d1dix[$i3]][$nd]+=($sftout-$nstart);}//echo"a15";
        else if($sbin<$nstart && $sftout>$midn){ $holarr[$d1dix[$i3]][$nd]+=($midn-$nstart);}//echo"a16";
       }
       ...
       if($slin<$nstart && $slout>$ynend){
        if($slin<$sftin){//early
         if($slin>=$ynend && $sftin<=$nstart){ $holarr[$d1dix[$i3]][$ot]+=($sftin-$slin);}//echo"b1";
         else if($slin<$ynend && $sftin>$ynend && $sftin<=$nstart){ $holarr[$d1dix[$i3]][$ot]+=($sftin-$ynend);}//echo"b2";
         else if($slin<$ynend && $sftin>$nstart){ $holarr[$d1dix[$i3]][$ot]+=($sftin-$ynend);}//echo"b3";
         else if($slin>=$ynend && $slin<$nstart && $sftin>$nstart){ $holarr[$d1dix[$i3]][$ot]+=($nstart-$slin); }
        }
        if($slout>$sftout){//ot
         if($sftout>=$ynend && $slout<=$nstart){ $holarr[$d1dix[$i3]][$ot]+=($slout-$sftout);}//echo"b4";
         else if($sftout>=$ynend && $sftout<$nstart && $slout>$nstart){ $holarr[$d1dix[$i3]][$ot]+=($nstart-$sftout);}//echo"b5";
         else if($sftout<$ynend && $slout>$nstart){ $holarr[$d1dix[$i3]][$ot]+=($nstart-$ynend);}//echo"b6";
        }
       }
      }
    }else if($day==2){
      for($i3=0;$i3<count($d2dix);$i3++){
       //new nd nextday
       if($sftin<=$midn && $sbout>$midn && $sbout<=$nend){ $holarr[$d2dix[$i3]][$nd]+=($sbout-$midn);}//ND nextday bo echo"s1=";
       else if($sftin>=$midn && $sbout<=$nend){ $holarr[$d2dix[$i3]][$nd]+=($sbout-$sftin);}//echo"s2";
       else if($sftin>=$midn && $sbout>$nend){ $holarr[$d2dix[$i3]][$nd]+=($nend-$sftin);}//echo"s3";
       else if($sftin>=$nend && $sbout>$nend){ $holarr[$d2dix[$i3]][$reg]+=($sbout-$sftin);}//echo"s4";
       else if($sftin<=$midn && $sbout>$midn && $sbout>$nend){ $holarr[$d2dix[$i3]][$nd]+=($nend-$midn); }

       if($sbin>=$midn && $sftout<=$nend){ $holarr[$d2dix[$i3]][$nd]+=($sftout-$sbin);} //ND nextday bi echo"s5"; 
       else if($sbin>=$midn && $sbin<=$nend && $sftout>$nend){ $holarr[$d2dix[$i3]][$nd]+=($nend-$sbin);}//echo"s6";
       else if($sftin<=$nend && $sbout>$nend){ $holarr[$d2dix[$i3]][$reg]+=($sbout-$nend);}//echo"s7";
       ...
       if($sbin<=$nend && $sftout>$nend){ $holarr[$d2dix[$i3]][$reg]+=($sftout-$nend);} //nextday regecho"s8"; 
       else if($sbin>=$nend && $sftout>$nend){ $holarr[$d2dix[$i3]][$reg]+=($sftout-$sbin);}//echo"s9";
       //new ot
       if($slout>$sftout && $slout>$midn){
        if($slout<=$nend && $sftout>=$midn){ $holarr[$d2dix[$i3]][$ndot]+=($slout-$sftout);}//nextday NDOTecho"s10";
        else if($slout<=$nend && $sftout<$midn){ $holarr[$d2dix[$i3]][$ndot]+=($slout-$midn);}//echo"s11";
        else if($slout>$nend && $sftout<$midn){ $holarr[$d2dix[$i3]][$ndot]+=($nend-$midn);}//echo"s12";
        else if($slout>$nend && $sftout>=$midn && $sftout<=$nend){ $holarr[$d2dix[$i3]][$ndot]+=($nend-$sftout); }

        if($slout>$nend && $sftout<=$nend){ $holarr[$d2dix[$i3]][$ot]+=($slout-$nend);}//nextday otecho"s13"; 
        else if($slout>$nend && $sftout>$nend){ $holarr[$d2dix[$i3]][$ot]+=($slout-$sftout);}//echo"s14";
       }
      }
    }
}}
...
$empsql=$link->query("SELECT `gy_emp_fullname`,`gy_emp_rate`,`gy_emp_type`,`gy_assignedloc` From `gy_employee` Where `gy_emp_code`='$sibsid' LIMIT 1");
$emprow=$empsql->fetch_array();
$fullname = $emprow['gy_emp_fullname'];
//$emprate = $emprow['gy_emp_rate'];
$emptyp = $emprow['gy_emp_type'];
       ...
       <table class="table table-hover table-bordered" style="font-family: 'Calibri'; font-size: 14px;">
            <thead>
                <tr style="padding:4px;" class="text-center text-nowrap bg-primary text-white">
                   ...
                   <th scope="col" >Absences<?php $lastrow[2]=0; ?></th>
...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='".$sibsid."' AND `gy_tracker_request`!='reject' AND `gy_tracker_request`!='' AND `gy_tracker_status`=1 ".$sqlseq." Order By `gy_tracker_date` ASC");
    while ($tsrow=$tmsht->fetch_array()){
        ...
        while($cntdate<date("Y-m-d", strtotime($truedate[0]))){
            ...
            //$tregot = "";
            //if($tsrow['gy_tracker_ath']>($truedate[1]-$tsrow['gy_tracker_bh'])){ $tregot=$tsrow['gy_tracker_ath']-($truedate[1]-$tsrow['gy_tracker_bh']); }
...
function tblcntt($mode, $sbsid, $date, $shift, $lin, $lout, $wh, $bh, $utl, $absnt, $htrw, $holarr, $lrow, $aprvnote, $empryt, $lstdt){
    ...
    if($loa[0]==1 && $mode==1){
        if($loa[2]==1){
            ...
            if($loa[4]==0.5 && ($in=="" || $out=="")){ //wo lilo
                //if($loa[3]==1){ $absnt=$loa[4]; }
                //else if($loa[3]==0){ $utl[0]=$loa[4]; $utl[1]=0; }
                if($empryt==1){ $absnt=$loa[4]; }
                else if($empryt==0){ $utl[0]=$loa[4]; $utl[1]=0; $utl[2]=0; }
    ...
    <tr class="text-nowrap <?php echo $rcolor; ?>">
        ...
        <td><?php $tmprw=0; if(strtotime($lout)<strtotime($lin)+32400 && $lin!="0000-00-00 00:00:00"&&$lout!="0000-00-00 00:00:00"){ $tmprw=getdstrdf((strtotime($lin)+32400)-strtotime($lout)); } $tmprw = ifzero(round($tmprw+$utl[2],9)); if($swh>=5){ if($tmprw>4&&$tmprw<=5){$tmprw=4;}else if($tmprw>5){$tmprw-=1;} } $lrow[1]+=$tmprw; echo $tmprw; ?></td>
        <td><?php $lrow[2]+=$absnt; echo $absnt; ?></td>
...
<?php return $lrow; } ?>
    ...
    <tr class="text-center text-nowrap bg-primary text-white">
        <th scope="col" style="padding-top:2px;padding-bottom:2px;">SIBS-<?php echo $sibsid; ?></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo $fullname; ?></th>
        <th></th>
        <th></th>
        <th></th>
        <th><input type="hidden" id="dtrhol" value="<?php echo $htrw; ?>"></th>

        <th style="padding-top:2px;padding-bottom:2px;"><?php echo $lastrow[0]; ?>
        <input type="hidden" id="dtrnof" value="<?php echo $lastrow[0]; ?>"></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo $lastrow[1]; ?>
        <input type="hidden" id="dtrltut" value="<?php echo $lastrow[1]; ?>"></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo $lastrow[2]; ?>
        <input type="hidden" id="dtrabcs" value="<?php echo $lastrow[2]; ?>"></th>

        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[3]); ?>
        <input type="hidden" id="dtrregot" value="<?php echo getdstrdf($lastrow[3]); ?>"></th><!-- Reg|OT -->
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[4]); ?>
        <input type="hidden" id="dtrrdreg" value="<?php echo getdstrdf($lastrow[4]); ?>"><!-- RD|Reg --></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[5]); ?>
        <input type="hidden" id="dtrrdot" value="<?php echo getdstrdf($lastrow[5]); ?>"><!-- RD|OT --></th>
    <?php $lrc=6; for($i=1;$i<$htrw;$i++){?>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholreg_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--|Reg--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholot_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--|OT--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholrdreg_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--|RD|Reg--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholrdot_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--|RD|OT--></th>
    <?php } ?>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrndreg" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND|Reg--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrndregot" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND|Reg|OT--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrndrdreg" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND|RD|Reg--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrndrdot" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND|RD|OT--></th>
    <?php for($i=1;$i<$htrw;$i++){?>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholnd_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND|--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholndot_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND||OT--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholndrd_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND||RD--></th>
        <th style="padding-top:2px;padding-bottom:2px;"><?php echo getdstrdf($lastrow[$lrc]); ?>
        <input type="hidden" id="dtrholndrdot_<?php echo $i; ?>" value="<?php echo getdstrdf($lastrow[$lrc]); $lrc++; ?>"><!--ND||RD|OT--></th>
    <?php } ?>
    </tr>
        	</tbody>
       </table>
