...
$requestdate="";
    ...
    $schsql=$link->query("SELECT `gy_tracker_login`,`gy_tracker_logout`,`gy_sched_login`,`gy_sched_logout`,`gy_sched_day`,`gy_req_date` FROM `gy_schedule_escalate` WHERE `gy_req_status`=1 AND `gy_sched_mode`=2 AND `gy_tracker_login`!='0000-00-00 00:00:00' AND `gy_tracker_logout`!='0000-00-00 00:00:00' AND `gy_sched_esc_code`=$trkcd ORDER BY `gy_sched_esc_id` desc Limit 1");
        while($schrow=$schsql->fetch_array()){
          if(strtotime($escrow['gy_req_date'])>strtotime($requestdate)){
            $lilo[0]=$escrow['gy_tracker_login'];
            $lilo[3]=$escrow['gy_tracker_logout'];
            $tmpin = date("Y-m-d H:i:s", strtotime(date("Y-m-d", strtotime($escrow['gy_sched_day']))." ".date("H:i:s",strtotime($escrow['gy_sched_login']))));
            $tmpout= date("Y-m-d H:i:s", strtotime(date("Y-m-d", strtotime($escrow['gy_sched_day']))." ".date("H:i:s",strtotime($escrow['gy_sched_logout']))));
...
function getholstatus($mode, $holdate, $shiftin, $shiftout, $lin, $lout, $bout, $bin, $hlarr){
    include '../../config/conn.php';
    $holarr = array(array());
...
$dssql=$link->query("SELECT * FROM `gy_holiday_calendar` LEFT JOIN `gy_holiday_types` ON `gy_holiday_calendar`.`gy_hol_type_id`=`gy_holiday_types`.`gy_hol_type_id` WHERE (`gy_holiday_calendar`.`gy_a_year`=1 AND `gy_holiday_calendar`.`gy_hol_date`='$hddate')OR(`gy_holiday_calendar`.`gy_a_year`=0 AND Year(`gy_holiday_calendar`.`gy_hol_date`)<='$curyear' AND (Year(`gy_holiday_calendar`.`gy_hol_lastday`)='0000' OR (Year(`gy_holiday_calendar`.`gy_hol_lastday`)!='0000' AND Year(`gy_holiday_calendar`.`gy_hol_lastday`)>='$curyear' ) ) )AND(MONTH(`gy_holiday_calendar`.`gy_hol_date`)='$curmonth'AND DAY(`gy_holiday_calendar`.`gy_hol_date`)='$curday') LIMIT 1");
        ...
        if($i==0){
            $d0dix = array_search($dsrow['gy_hol_type_id'],$hlarr);
            $ystart = strtotime($dayarr[$i]." ".$dsrow['gy_day_start']);
            $yend = strtotime($dayarr[$i]." ".$dsrow['gy_day_end']);
            $ynstart = strtotime($dayarr[$i]." ".$dsrow['gy_night_start']);
            $ynend = strtotime($dayarr[$i]." ".$dsrow['gy_night_end']);
            if(mysqli_num_rows($dssql)>0){
             if($ynstart>$ynend){ $ynend=strtotime($dayarr[$i]." ".$dsrow['gy_night_end']." +1 day"); }
            }else{
            ...
            $start = strtotime($dayarr[$i]." ".$dsrow['gy_day_start']);
            $end = strtotime($dayarr[$i]." ".$dsrow['gy_day_end']);
            $nstart = strtotime($dayarr[$i]." ".$dsrow['gy_night_start']);
            $nend = strtotime($dayarr[$i]." ".$dsrow['gy_night_end']);
            $d1dix = array_search($dsrow['gy_hol_type_id'],$hlarr);
            if(mysqli_num_rows($dssql)>0){
             if($nstart>$nend){ $nend=strtotime($dayarr[$i]." ".$dsrow['gy_night_end']." +1 day"); }
            }else{
            ...
            $d2dix = array_search($dsrow['gy_hol_type_id'],$hlarr);
            if($d2dix==""){ $d2dix=0; }
        }
}
    $link->close();
...
if($mode==1 || $mode==2){ for($day=0;$day<count($dayarr);$day++){
    if($day==0){
        if($slin<=$ymidn && $slin>=$ynstart && $sbout>=$ymidn){ $holarr[$d0dix][$ndot]+=($ymidn-$slin); }//early NDOT
        else if($sbout<=$ymidn && $slin>=$ynstart){ $holarr[$d0dix][$ndot]+=($sbout-$slin); }//early NDOT
        else if($sbout>$ymidn && $slin<$ynstart){ $holarr[$d0dix][$ndot]+=($ymidn-$ynstart); }//early NDOT
        else if($slin<$ynstart && $sbout>=$ynstart && $sbout<=$ymidn){ $holarr[$d0dix][$ndot]+=($sbout-$ynstart); }//early NDOT

        if($sbin<=$ymidn && $sbin>=$ynstart && $slout>$ymidn){ $holarr[$d0dix][$ndot]+=($ymidn-$sbin); }//early NDOT
        else if($sbin<=$ynstart){ $holarr[$d0dix][$ndot]+=($ymidn-$ynstart); }//early NDOT

        if($slin<$ynstart && $sbout>=$ynstart){ $holarr[$d0dix][$ot]+=($ynstart-$slin); }//early OT
        else if($slin<$ynstart && $sbout<=$ynstart){ $holarr[$d0dix][$ot]+=($sbout-$slin); }//early OT
        else if($sbin<$ynstart){ $holarr[$d0dix][$ot]+=($ynstart-$sbin); }//early OT
    }else if($day==1){
       // morning ND
       if($sftout>$ymidn && $sftin<$ynend){
        if($sftin>=$ymidn && $sbout<=$ynend){ $holarr[$d1dix][$nd]+=($sbout-$sftin); } //ND BO
        else if($sftin<$ymidn && $sbout<=$ynend && $sbout>$ymidn){ $holarr[$d1dix][$nd]+=($sbout-$ymidn); }
        else if($sftin>=$ymidn && $sftin<$ynend && $sbout>=$ynend){ $holarr[$d1dix][$nd]+=($ynend-$sftin); }
        else if($sftin>$ymidn && $sbout>$ynend){ $holarr[$d1dix][$nd]+=($ynend-$ymidn); }

        if($sbin>=$ymidn && $sftout<=$ynend){ $holarr[$d1dix][$nd]+=($sftout-$sbin); } //ND BI
        else if($sbin>=$ymidn && $sbin<$ynend && $sftout>$ynend){ $holarr[$d1dix][$nd]+=($ynend-$sbin); }
        else if($sbin<=$ymidn && $sftout>$ymidn && $sftout<=$ynend){ $holarr[$d1dix][$nd]+=($sftout-$ymidn); }
        else if($sbin<$ymidn && $sftout>$ynend){ $holarr[$d1dix][$nd]+=($ynend-$ymidn); }
       }
       // night ND
       if($sftin<$midn && $sftout>$nstart){
        if($sftin>=$nstart && $sbout<=$midn){ $holarr[$d1dix][$nd]+=($sbout-$sftin); }//ND BO
        else if($sftin>=$nstart && $sftin<$midn && $sbout>$midn){ $holarr[$d1dix][$nd]+=($midn-$sftin); }
        else if($sftin<$nstart && $sbout>$nstart && $sbout<=$midn){ $holarr[$d1dix][$nd]+=($sbout-$nstart); }
        else if($sftin<$nstart && $sbout>$midn){ $holarr[$d1dix][$nd]+=($midn-$nstart); }

        if($sbin>=$nstart && $sftout<=$midn){ $holarr[$d1dix][$nd]+=($sftout-$sbin); }//ND BI
        else if($sbin>=$nstart && $sbin<$midn && $sftout>$midn){ $holarr[$d1dix][$nd]+=($midn-$sbin); }
        else if($sbin<$nstart && $sftout>$nstart && $sftout<=$midn){ $holarr[$d1dix][$nd]+=($sftout-$nstart); }
        else if($sbin<$nstart && $sftout>$midn){ $holarr[$d1dix][$nd]+=($midn-$nstart); }
       }

       // morning NDOT
       if($slin<$ynend && $slout>$ymidn){
        if($slin<$sftin){//NDOT early
         if($slin>=$ymidn && $sftin<=$ynend){ $holarr[$d1dix][$ndot]+=($sftin-$slin); }
         else if($slin<$ymidn && $sftin<=$ynend && $sftin>$ymidn){ $holarr[$d1dix][$ndot]+=($sftin-$ymidn); }
         else if($slin>=$ymidn && $slin<$ynend && $sftin>$ynend){ $holarr[$d1dix][$ndot]+=($ynend-$slin); }
         else if($slin<$ymidn && $sftin>$ynend){ $holarr[$d1dix][$ndot]+=($ynend-$ymidn); }
        }
        if($slout>$sftout){ //NDOT OT
         if($sftout>=$ymidn && $slout<=$ynend){ $holarr[$d1dix][$ndot]+=($slout-$sftout); }
         else if($sftout>=$ymidn && $sftout<=$ynend && $slout>$ynend){ $holarr[$d1dix][$ndot]+=($ynend-$sftout); }
        }
       }
       // evening NDOT
       if($slin<$midn && $slout>$nstart){
        if($slin<$sftin){//NDOT early
         if($slin>=$nstart && $sftin<=$midn){ $holarr[$d1dix][$ndot]+=($sftin-$slin); }
         else if($slin<$nstart && $sftin>$nstart && $sftin<$midn){ $holarr[$d1dix][$ndot]+=($sftin-$nstart); }
        }
        if($slout>$sftout){//NDOT OT
         if($sftout>=$nstart && $slout<=$midn){ $holarr[$d1dix][$ndot]+=($slout-$sftout); }
         else if($sftout>=$nstart && $sftout<$midn && $slout>$midn){ $holarr[$d1dix][$ndot]+=($midn-$sftout); }
         else if($sftout<$nstart && $slout>$midn){ $holarr[$d1dix][$ndot]+=($midn-$nstart); }
         else if($sftout<$nstart && $slout>$nstart && $slout<=$midn){ $holarr[$d1dix][$ndot]+=($slout-$nstart); }
        }
       }
       // regular
       if($sftin<$nstart && $sftout>$ynend){
        if($sftin>=$ynend && $sbout<=$nstart){ $holarr[$d1dix][$reg]+=($sbout-$sftin); }//reg bo
        else if($sftin<$ynend && $sbout>$ynend && $sbout<=$nstart){ $holarr[$d1dix][$reg]+=($sbout-$ynend); }
        else if($sftin>=$ynend && $sftin<$nstart && $sbout>$nstart){ $holarr[$d1dix][$reg]+=($nstart-$sftin); }
        else if($sftin<$ynend && $sbout>$nstart){ $holarr[$d1dix][$reg]+=($nstart-$ynend); }

        if($sbin>=$ynend && $sftout<=$nstart){ $holarr[$d1dix][$reg]+=($sftout-$sbin); }//reg bi
        else if($sbin>=$ynend && $sbin<$nstart && $sftout>$nstart){ $holarr[$d1dix][$reg]+=($nstart-$sbin); }
        else if($sbin<$ynend && $sftout>$ynend && $sftout<=$nstart){ $holarr[$d1dix][$reg]+=($sftout-$ynend); }
        else if($sbin<$ynend && $sftout>$nstart){ $holarr[$d1dix][$reg]+=($nstart-$ynend); }
       }
       // regular ot
       if($slin<$nstart && $slout>$ynend){
        if($slin<$sftin){//early
         if($slin>=$ynend && $sftin<=$nstart){ $holarr[$d1dix][$ot]+=($sftin-$slin); }
         else if($slin<$ynend && $sftin>$ynend && $sftin<=$nstart){ $holarr[$d1dix][$ot]+=($sftin-$ynend); }
         else if($slin<$ynend && $sftin>$nstart){ $holarr[$d1dix][$ot]+=($sftin-$ynend); }
         else if($slin>=$ynend && $slin<$nstart && $sftin>$nstart){ $holarr[$d1dix][$ot]+=($nstart-$slin); }
        }
        if($slout>$sftout){//ot
         if($sftout>=$ynend && $slout<=$nstart){ $holarr[$d1dix][$ot]+=($slout-$sftout); }
         else if($sftout>=$ynend && $sftout<$nstart && $slout>$nstart){ $holarr[$d1dix][$ot]+=($nstart-$sftout); }
         else if($sftout<$ynend && $slout>$nstart){ $holarr[$d1dix][$ot]+=($nstart-$ynend); }
        }
       }
    }else if($day==2){
       //nd nextday
       if($sftin<=$midn && $sbout>$midn && $sbout<=$nend){ $holarr[$d2dix][$nd]+=($sbout-$midn); }//ND nextday bo
       else if($sftin>=$midn && $sbout<=$nend){ $holarr[$d2dix][$nd]+=($sbout-$sftin); }
       else if($sftin>=$midn && $sbout>$nend){ $holarr[$d2dix][$nd]+=($nend-$sftin); }
       else if($sftin>=$nend && $sbout>$nend){ $holarr[$d2dix][$reg]+=($sbout-$sftin); }

       if($sbin>=$midn && $sftout<=$nend){ $holarr[$d2dix][$nd]+=($sftout-$sbin); } //ND nextday bi
       else if($sbin>=$midn && $sbin<=$nend && $sftout>$nend){ $holarr[$d2dix][$nd]+=($nend-$sbin); }
       else if($sftin<=$nend && $sbout>$nend){ $holarr[$d2dix][$reg]+=($sbout-$nend); }
       else if($sbin<=$midn && $sftout>$midn && $sftout<=$nend){ $holarr[$d2dix][$nd]+=($sftout-$midn); }
       else if($sbin<=$midn && $sftout>$nend){ $holarr[$d2dix][$nd]+=($nend-$midn); }

       if($sbin<=$nend && $sftout>$nend){ $holarr[$d2dix][$reg]+=($sftout-$nend); } //nextday reg
       else if($sbin>=$nend && $sftout>$nend){ $holarr[$d2dix][$reg]+=($sftout-$sbin); }
       //ot/ndot
       if($slout>$sftout && $slout>$midn){
        if($slout<=$nend && $sftout>=$midn){ $holarr[$d2dix][$ndot]+=($slout-$sftout); }//nextday NDOT
        else if($slout<=$nend && $sftout<$midn){ $holarr[$d2dix][$ndot]+=($slout-$midn); }
        else if($slout>$nend && $sftout<$midn){ $holarr[$d2dix][$ndot]+=($nend-$midn); }
        else if($slout>$nend && $sftout>=$midn && $sftout<=$nend){ $holarr[$d2dix][$ndot]+=($nend-$sftout); }

        if($slout>$nend && $sftout<=$nend){ $holarr[$d2dix][$ot]+=($slout-$nend); }//nextday ot
        else if($slout>$nend && $sftout>$nend){ $holarr[$d2dix][$ot]+=($slout-$sftout); }
       }
    }
}}
    return $holarr;    
...
$empsql=$link->query("SELECT `gy_emp_fullname`,`gy_emp_rate` From `gy_employee` Where `gy_emp_code`='$sibsid' LIMIT 1");
...
$tmsht=$link->query("SELECT * From `gy_tracker` Where `gy_emp_code`='".$sibsid."' AND `gy_tracker_request`!='reject' AND `gy_tracker_status`=1 ".$sqlseq." Order By `gy_tracker_date` ASC");
    while ($tsrow=$tmsht->fetch_array()) {
        ...
        if(date("Y-m-d", strtotime($truedate[0]))>=date('Y-m-d', strtotime($datefrom)) && date("Y-m-d", strtotime($truedate[0]))<=date('Y-m-d', strtotime($dateto))){
            $dayholsta = array(array());
            $dayholsta = getholstatus($thismode, date("Y-m-d", strtotime($truedate[0])), $truedate[2], $truedate[3], $lilo[0], $lilo[3], $lilo[1], $lilo[2], $hlarr);
...
?>
    <tr class="text-nowrap <?php echo $rcolor; ?>">
        <td><?php echo $abrv[0];
            if($utl[2]>0){ if($aprvnote!=""){$aprvnote.=" | ";} $aprvnote.="Overbreak"; }
