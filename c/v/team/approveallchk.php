<?php
	...
	$trackids = explode(",",addslashes($_REQUEST['trackids']));
    $ahtnoot = addslashes($_REQUEST['ahtnoot']);
	$ifcor=$link->query("SELECT `gy_emp_code` From `gy_employee` LEFT JOIN `gy_accounts` ON `gy_employee`.`gy_acc_id`=`gy_accounts`.`gy_acc_id` Where (`gy_employee`.`gy_emp_supervisor`='$user_id' OR `gy_accounts`.`gy_dept_id`=2) AND `gy_employee`.`gy_emp_type`<$user_type AND `gy_employee`.`gy_emp_code`='$nameid' LIMIT 1");
	if(mysqli_num_rows($ifcor) > 0){
		for($i=0;$i<count($trackids);$i++){
			if (check_pending($trackids[$i]) == "yes") {
			$ifcorin=$link->query("SELECT `gy_emp_code`, `gy_tracker_id`, `gy_tracker_login`, `gy_tracker_logout`, gy_tracker_breakout, gy_tracker_breakin, `gy_tracker_bh`, `gy_tracker_ot`, `gy_tracker_ath` From `gy_tracker` Where `gy_emp_code`='$nameid' AND `gy_tracker_id`='$trackids[$i]' AND `gy_tracker_request`='' LIMIT 1");
			if(mysqli_num_rows($ifcor) > 0){
			$fcrow=$ifcorin->fetch_array();
			...
			}else if($ahtnoot == 'whwot'){
			$thisath = getathnoot($fcrow['gy_tracker_logout'], $thewh[1], $fcrow['gy_tracker_bh'], $theutl, $fcrow['gy_tracker_ath']);
			}
			if($thisath > 0 && $fcrow['gy_tracker_ath'] == 0 && $thewh[2]){
			$updatedata=$link->query("UPDATE `gy_tracker` SET `gy_tracker_ath`='$thisath',`gy_tracker_request`='approve',`gy_tracker_om`='$user_id' Where `gy_tracker_id`='$trackids[$i]'");
			}
			}}
		}
	}
