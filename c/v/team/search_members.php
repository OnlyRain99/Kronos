...
include 'session.php';
 ...
 $getlst=$link->query("SELECT `gy_emp_fullname`, `gy_emp_code` From `gy_employee` LEFT JOIN `gy_user` ON `gy_employee`.`gy_emp_code`=`gy_user`.`gy_user_code` Where `gy_user`.`gy_user_status`=0 AND `gy_employee`.`gy_acc_id`='$cntrlval' ");
