    ...
    if(date("d")<=3){ $datestrt = date("Y-m-16", strtotime("-1 Month")); }
        ...
        $today = date("Y-m-d");
            ...
            $i = 0;
                ...
                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_acc_id`='$myaccount' OR `gy_employee`.`gy_emp_supervisor`='$user_id') AND `gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']." AND `gy_leave`.`gy_leave_status`=0 ");
                ...
                $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_acc_id`='$myaccount' OR  `gy_employee`.`gy_emp_supervisor`='$user_id') AND (`gy_leave`.`gy_leave_date_from`>='$today' OR `gy_leave`.`gy_leave_date_approved`>='$today') AND `gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']." AND `gy_leave`.`gy_leave_status`=1 ");
                while($glsrow=$ctlsql->fetch_array()){
                    $own='Team';
                    if($glsrow['gy_user_id']==$user_id){ $own='My';  }
                    cardloacont($glsrow['gy_leave_id'], $glsrow['gy_leave_status'], $own, $glsrow['gy_emp_fullname'], $glsrow['gy_leave_filed'], get_leave_type($glsrow['gy_leave_type']), $glsrow['gy_leave_date_from'], $glsrow['gy_leave_reason'], $glsrow['gy_leave_date_approved'], getuserfullname($glsrow['gy_leave_approver']), $glsrow['gy_leave_remarks']);
               ...
               $ctlsql=$link->query("SELECT * FROM `gy_leave` LEFT JOIN `gy_user` ON `gy_leave`.`gy_user_id`=`gy_user`.`gy_user_id` JOIN `gy_employee` ON `gy_user`.`gy_user_code`=`gy_employee`.`gy_emp_code` WHERE (`gy_leave`.`gy_acc_id`='$myaccount' OR  `gy_employee`.`gy_emp_supervisor`='$user_id') AND (`gy_leave`.`gy_leave_date_from`>='$today' OR `gy_leave`.`gy_leave_date_approved`>='$today') AND `gy_user`.`gy_user_type`<".$_SESSION['fus_user_type']." AND `gy_leave`.`gy_leave_status`=2 ");
                while($glsrow=$ctlsql->fetch_array()){
                    $own='Team';
                    if($glsrow['gy_user_id']==$user_id){ $own='My';  }
                    cardloacont($glsrow['gy_leave_id'], $glsrow['gy_leave_status'], $own, $glsrow['gy_emp_fullname'], $glsrow['gy_leave_filed'], get_leave_type($glsrow['gy_leave_type']), $glsrow['gy_leave_date_from'], $glsrow['gy_leave_reason'], $glsrow['gy_leave_date_approved'], getuserfullname($glsrow['gy_leave_approver']), $glsrow['gy_leave_remarks']);
            ...
            function cardloacont($cardn, $reqstatus, $own, $fname, $fyldte, $loanm, $loadate, $reason, $rspdate, $rspby, $rmrks){
                ...
                function cardcontent($cardn, $esct, $own, $reqdate, $fname, $reqstatus, $reqdeny, $reqby, $reqto, $sup, $schmode, $schday, $schli, $schbo, $schbi, $schlo, $reqreson){
                ...
                <div class="col-md-4">
                <div class="card hover-overlay shadow rounded">
                    <div class="card-header mask <?php if($color=="warning"){echo "bg-secondary";}else{ echo "bg-".$color; } ?>"><h4 class="text-center text-light"><?php echo $ctitle; ?></h4></div>
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item">
                        <div class="position-relative m-4">
                            <div class="progress" style="height: 1px;">
