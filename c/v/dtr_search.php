...
$sqlby = "";
	...
	$empsql=$link->query("SELECT * From `gy_employee` LEFT JOIN `gy_user` ON `gy_employee`.`gy_emp_code`=`gy_user`.`gy_user_code` Where ".$likename." ".$sqlby." ".$sqlsts." ORDER BY `gy_employee`.`gy_emp_lname` asc");
	while($emprow=$empsql->fetch_array()){
		...
		cmpltbl($emprow['gy_user_code'], $emprow['gy_emp_lname'].", ".$emprow['gy_emp_fname'], $emprow['gy_user_type'], $emprow['gy_user_status'], $publish, $dmrate, $cmpute);
	}
