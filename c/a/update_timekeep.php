<?php  
    ...
    if (isset($_POST['logindate'])) {
        ...
        if ($clogin == "" && $cbreakout == "" && $cbreakin == "" && $clogout == "") {
            if ($mode == "normal") {
                header("location: masterlist?note=nochange");
            }else{
                header("location: search_master?note=nochange&s=$s&f=$f&t=$t&a=$a&b=$b");
            }
        }else{
            //update timekeep
            $updatedata=$link->query("UPDATE `gy_tracker` SET `gy_tracker_date`='$login',`gy_tracker_login`='$login',`gy_tracker_breakout`='$breakout',`gy_tracker_breakin`='$breakin',`gy_tracker_logout`='$logout',`gy_tracker_wh`='$wh',`gy_tracker_bh`='$bh',`gy_tracker_ot`='$ot',`gy_tracker_history`=CONCAT(`gy_tracker_history`,'$updates') Where `gy_tracker_id`='$redirect'");
            ...
            if ($updatedata) {
                ...
                if ($mode == "normal") {
                    header("location: masterlist?note=update");
                }else{
                    header("location: search_master?note=update&s=$s&f=$f&t=$t&a=$a&b=$b");
                }
