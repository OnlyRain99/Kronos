<?php 
    ...
    $title = "System Users Search: ".$search_text;

    $query_one = "SELECT * From `gy_user` Where `gy_user_id`!='$user_id' AND `gy_user_id`!='1' AND CONCAT(`gy_full_name`,`gy_username`) LIKE '%$search_text%' Order By `gy_full_name` ASC";

    $query_two = "SELECT COUNT(`gy_user_id`) FROM `gy_user` Where `gy_user_id`!='$user_id' AND `gy_user_id`!='1' AND CONCAT(`gy_full_name`,`gy_username`) LIKE '%$search_text%' Order By `gy_full_name` ASC";

    $query_three = "SELECT * From `gy_user` Where `gy_user_id`!='$user_id' AND `gy_user_id`!='1' AND CONCAT(`gy_full_name`,`gy_username`) LIKE '%$search_text%' Order By `gy_full_name` ASC ";

    $my_num_rows = 50;

    //get accounts
    $info=$link->query("SELECT * From `gy_user` Where `gy_user_id`!='$user_id' AND `gy_user_id`!='1' AND CONCAT(`gy_full_name`,`gy_username`) LIKE '%$search_text%' Order By `gy_full_name` ASC");
        ...
        <div class="page-container">
            ...
            <div class="main-content" style="padding: 20px;">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="title-1 m-b-25"><?php echo $title; ?> <i class="fas fa-unlock"></i></h2>
                        </div>
                    </div>
                                        ...
                                        <table class="table table-bordered" style="font-family: 'Calibri'; font-size: 14px;">
                                            <thead>
                                                <tr class="mybg">
                                                    <th>Name</th>
                                                    <th>Username</th>
                                                    <th style="padding:0px" class="text-center">Edit</th>
                                                    <th style="padding:0px" class="text-center">Delete</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php  
                                                ...
                                                <tr class="mybg">
                                                    ...
                                                    <td class="text-center" style="padding: 0px;"><button type="button" data-toggle="modal" data-target="#edit_<?php echo $usersrow['gy_user_id']; ?>" class="btn btn-info btn-sm btn-block" title="click to edit ..."><i class="fa fa-edit"></i></button></td>
                                                    <td class="text-center" style="padding: 0px;"><button type="button" data-toggle="modal" data-target="#delete_<?php echo $usersrow['gy_user_id']; ?>" class="btn btn-danger btn-sm btn-block" title="click to delete ..."><i class="fa fa-trash"></i></button></td>
                                                </tr>
